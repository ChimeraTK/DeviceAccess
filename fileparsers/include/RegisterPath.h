...
public:
  ...
  RegisterPath(const std::string &_path)
      : path(std::string(separator + _path)) {
    removeExtraSeparators();
  }
  RegisterPath(const RegisterPath &_path)
      : path(_path.path), separator_alt(_path.separator_alt) {
    removeExtraSeparators();
  }
  RegisterPath(const char *_path) : path(std::string(separator) + _path) {
    removeExtraSeparators();
  }

  /** type conversion operators into std::string */
  operator std::string() const {
    return getWithOtherSeparatorReplaced(separator_alt);
  }
  ...
  bool operator<(const RegisterPath &rightHandSide) const {
    ...
    return getWithOtherSeparatorReplaced(sepalt) <
           rightHandSide.getWithOtherSeparatorReplaced(sepalt);
  }
  ...
  RegisterPath &operator--(int) {
    ...
    if (found != std::string::npos &&
        found > 0) { // don't find the leading separator...
  ...
  RegisterPath &operator--() {
    std::size_t found = path.find_first_of(
        std::string(separator), 1); // don't find the leading separator...
    if (found != std::string::npos) {
  ...
  bool startsWith(const RegisterPath &compare) const {
    ...
    std::string otherPathConverted(
        compare.getWithOtherSeparatorReplaced(sepalt));
  ...
  std::vector<std::string> getComponents() const {
    ...
    while (pos != std::string::npos) {
      ...
      if (separator_alt.length() != 0) {
        ...
        if (npos_alt != std::string::npos && npos_alt < npos)
          npos = npos_alt;
      }
  ...
  friend RegisterPath operator/(const RegisterPath &leftHandSide,
...
protected:
  ...
  std::string removeExtraSeparators(std::string string,
                                    std::string sep = separator) const {
    std::size_t pos;
    while ((pos = string.find(std::string(sep) + sep)) != std::string::npos) {
      string.erase(pos, 1);
    }
    if (string.length() > 1 && string.substr(string.length() - 1, 1) == sep)
   ...
   * separator "/". */
  std::string
  getWithOtherSeparatorReplaced(const std::string &otherSeparator) const {
...
std::string operator+(const std::string &leftHandSide,
                      const RegisterPath &rightHandSide);
RegisterPath operator+(const RegisterPath &leftHandSide,
                       const std::string &rightHandSide);
