...
DeviceInfoMapPointer DMapFileParser::parse(const std::string &file_name) {
  ...
  if (!file) {
    throw ChimeraTK::logic_error("Cannot open dmap file: \"" +
                                 absPathToDMapFile + "\"");
  }
  ...
  if (dmap->getSize() == 0) {
    ...
    throw ChimeraTK::detail::EmptyDMapFileException(
        "No data in in dmap file: \"" + file_name + "\"");
  }
  return dmap;
}
...
void DMapFileParser::parseRegularLine(std::string file_name, std::string line,
  ...
  if (inStream) {
    std::string absPathToDMapFile = utilities::convertToAbsolutePath(file_name);
    std::string absPathToMapFile =
        absPathOfDMapContent(deviceInfo.mapFileName, file_name);
    ...
    if (inStream2) {
      std::string absPathToDMapFile =
          utilities::convertToAbsolutePath(file_name);
...
std::string DMapFileParser::absPathOfDMapContent(std::string dmapContent,
                                                 std::string dmapFileName) {
  // first determine the absolute path to the dmap file
  std::string absPathToDMapFile =
      utilities::convertToAbsolutePath(dmapFileName);
  // then extract the directory
  std::string absPathToDmapDirectory =
      utilities::extractDirectory(absPathToDMapFile);
...
void DMapFileParser::raiseException(std::string file_name, std::string line,
                                    uint32_t line_nr) {
  std::stringstream errorMessage;
  errorMessage << "Error in dmap file: \"" << file_name << "\" in line ("
               << line_nr << ") \"" << line << "\"";
