...
void DeviceInfoMap::getDeviceInfo(const std::string &deviceName,
                                  DeviceInfo &value) {
  std::vector<DeviceInfo>::iterator iter;
  iter = find_if(_deviceInfoElements.begin(), _deviceInfoElements.end(),
                 findDevByName_pred(deviceName));
  if (iter == _deviceInfoElements.end()) {
    throw ChimeraTK::logic_error("Cannot find device \"" + deviceName +
...
DeviceInfoMap::DeviceInfo::getDeviceFileAndMapFileName() const {
  return std::pair<std::string, std::string>(uri, mapFileName);
}

std::ostream &operator<<(std::ostream &os,
                         const DeviceInfoMap::DeviceInfo &deviceInfo) {
  os << "(" << deviceInfo.dmapFileName << ") NAME: " << deviceInfo.deviceName
     << " DEV : " << deviceInfo.uri << " MAP : " << deviceInfo.mapFileName;
  return os;
}

// fixme: why is level not used?
bool DeviceInfoMap::check(DeviceInfoMap::ErrorList &err,
                          DeviceInfoMap::ErrorList::ErrorElem::TYPE /*level*/) {
  ...
  while (1) {
    if ((*iter_p).deviceName == (*iter_n).deviceName) {
      if ((*iter_p).uri != (*iter_n).uri ||
          (*iter_p).mapFileName != (*iter_n).mapFileName) {
        err.insert(ErrorList::ErrorElem(
            ErrorList::ErrorElem::ERROR,
            ErrorList::ErrorElem::NONUNIQUE_DEVICE_NAME, (*iter_p), (*iter_n)));
        ret = false;
      }
    }
    iter_p = iter_n;
...
DeviceInfoMap::ErrorList::ErrorElem::ErrorElem(
    TYPE infoType, DMAP_FILE_ERR errorType,
    const DeviceInfoMap::DeviceInfo &device1,
...
std::ostream &operator<<(std::ostream &os,
  ...
  case DeviceInfoMap::ErrorList::ErrorElem::NONUNIQUE_DEVICE_NAME:
    os << me._type
       ...
       << me._errorDevice1.deviceName << "\" in file \""
       << me._errorDevice1.dmapFileName << "\" in line "
       << me._errorDevice1.dmapFileLineNumber << " and \""
       << me._errorDevice2.dmapFileName << "\" in line "
