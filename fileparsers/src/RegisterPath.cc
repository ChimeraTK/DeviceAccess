...
RegisterPath operator/(const RegisterPath &leftHandSide,
  ...
  leftHandSide.getCommonAltSeparator(
      rightHandSide); // just to check compatibility of the two RegisterPaths
...
RegisterPath operator*(const RegisterPath &leftHandSide, int rightHandSide) {
  RegisterPath ret(std::string(leftHandSide) + std::string("*") +
                   std::to_string(rightHandSide));
