...
std::ostream &operator<<(std::ostream &os,
                         const RegisterInfoMap &registerInfoMap) {
  os << "=======================================" << std::endl;
  os << "MAP FILE NAME: " << registerInfoMap._mapFileName << std::endl;
  os << "---------------------------------------" << std::endl;
  for (auto it = registerInfoMap._catalogue.metadata_begin();
       it != registerInfoMap._catalogue.metadata_end(); ++it) {
    os << "METADATA-> NAME: \"" << it->first << "\" VALUE: " << it->second
       << std::endl;
  }
  os << "---------------------------------------" << std::endl;
...
void RegisterInfoMap::insert(RegisterInfo &elem) {
  _catalogue.addRegister(
      boost::shared_ptr<RegisterInfo>(new RegisterInfo(elem)));
}
...
void RegisterInfoMap::getRegisterInfo(const std::string &reg_name,
  ...
  auto infoCast =
      boost::static_pointer_cast<RegisterInfoMap::RegisterInfo>(info);
  value = *infoCast;
}
...
void RegisterInfoMap::getRegisterInfo(int reg_nr, RegisterInfo &value) const {
  ...
  for (auto it = _catalogue.begin(); it != _catalogue.end(); ++it) {
    if (count == reg_nr) {
      auto infoCast =
          boost::static_pointer_cast<const RegisterInfoMap::RegisterInfo>(
              it.get());
      value = *infoCast;
      return;
    }
...
void RegisterInfoMap::getMetaData(const std::string &metaDataName,
                                  std::string &metaDataValue) const {
...
bool RegisterInfoMap::check(ErrorList &err,
                            RegisterInfoMap::ErrorList::ErrorElem::TYPE level) {
  ...
  for (auto it = _catalogue.begin(); it != _catalogue.end(); ++it) {
    auto infoCast =
        boost::static_pointer_cast<RegisterInfoMap::RegisterInfo>(it.get());
  ...
  if (map_file.size() < 2)
    return true;
  std::sort(map_file.begin(), map_file.end(),
            compareRegisterInfosByName_functor());
  iter_p = map_file.begin();
  iter_n = iter_p + 1;
  while (1) {
    if ((iter_p->name == iter_n->name) && (iter_p->module == iter_n->module)) {
      err.insert(
          ErrorList::ErrorElem(ErrorList::ErrorElem::ERROR,
        ...
        if ((v_iter->start >= iter_p->address) &&
          ...
          if (level >= ErrorList::ErrorElem::WARNING) {
            err.insert(ErrorList::ErrorElem(
                ErrorList::ErrorElem::WARNING,
                ErrorList::ErrorElem::WRONG_REGISTER_ADDRESSES, *iter_p,
                *(v_iter->iter), _mapFileName));
            ret = false;
          }
        } else if ((v_iter->start <= iter_p->address) &&
                   (iter_p->address < v_iter->end)) {
          if (level >= ErrorList::ErrorElem::WARNING) {
            err.insert(ErrorList::ErrorElem(
                ErrorList::ErrorElem::WARNING,
                ErrorList::ErrorElem::WRONG_REGISTER_ADDRESSES, *iter_p,
...
std::ostream &operator<<(std::ostream &os,
                         const RegisterInfoMap::MetaData &me) {
  os << "METADATA-> NAME: \"" << me.name << "\" VALUE: " << me.value
     << std::endl;
  return os;
}

std::ostream &operator<<(std::ostream &os,
                         const RegisterInfoMap::RegisterInfo::Type &type) {
  switch (type) {
...
std::ostream &operator<<(std::ostream &os,
                         const RegisterInfoMap::RegisterInfo::Access &access) {
  switch (access) {
...
std::ostream &operator<<(std::ostream &os,
  ...
  os << registerInfo.name << " 0x" << std::hex << registerInfo.nElements
     << " 0x" << registerInfo.address << " 0x" << registerInfo.nBytes << " 0x"
     << registerInfo.bar << std::dec << " " << registerInfo.width << " "
     << registerInfo.nFractionalBits << " "
...
operator<<(std::ostream &os,
           const RegisterInfoMap::ErrorList::ErrorElem::TYPE &me) {
  switch (me) {
...
RegisterInfoMap::ErrorList::ErrorElem::ErrorElem(
    RegisterInfoMap::ErrorList::ErrorElem::TYPE info_type,
    RegisterInfoMap::ErrorList::ErrorElem::MAP_FILE_ERR e_type,
    const RegisterInfoMap::RegisterInfo &reg_1,
...
std::ostream &operator<<(std::ostream &os,
                         const RegisterInfoMap::ErrorList::ErrorElem &me) {
  switch (me._errorType) {
  case RegisterInfoMap::ErrorList::ErrorElem::NONUNIQUE_REGISTER_NAME:
    os << me._type << ": Found two registers with the same name: \""
       << me._errorRegister1.name << "\" in file " << me._errorFileName;
    break;
  case RegisterInfoMap::ErrorList::ErrorElem::WRONG_REGISTER_ADDRESSES:
    os << me._type << ": Found two registers with overlapping addresses: \""
       << me._errorRegister1.name << "\" and \"" << me._errorRegister2.name
       << "\" in file " << me._errorFileName;
    break;
  }
  return os;
}
...
void RegisterInfoMap::ErrorList::insert(
    const RegisterInfoMap::ErrorList::ErrorElem &elem) {
  errors.push_back(elem);
}

std::ostream &operator<<(std::ostream &os,
                         const RegisterInfoMap::ErrorList &me) {
...
std::list<RegisterInfoMap::RegisterInfo>
RegisterInfoMap::getRegistersInModule(std::string const &moduleName) {
  ...
  std::sort(sortedRegisterInfos.begin(), sortedRegisterInfos.end(),
            compareRegisterInfosByName_functor());
  ...
  std::vector<RegisterInfo>::iterator firstMatchingIterator =
      std::find_if(sortedRegisterInfos.begin(), sortedRegisterInfos.end(),
                   compareModuleName_pred(moduleName));
...
RegisterInfoMap::RegisterInfo::RegisterInfo(
    std::string const &name_, uint32_t nElements_, uint32_t address_,
    uint32_t nBytes_, uint32_t bar_, uint32_t width_, int32_t nFractionalBits_,
    bool signedFlag_, std::string const &module_, uint32_t nChannels_,
    bool is2DMultiplexed_, Access dataAccess_, Type dataType_)
    : name(name_), nElements(nElements_), nChannels(nChannels_),
      is2DMultiplexed(is2DMultiplexed_), address(address_), nBytes(nBytes_),
      bar(bar_), width(width_), nFractionalBits(nFractionalBits_),
  ...
  if (dataType == IEEE754) {
    if (width == 32) {
      ...
      dataDescriptor = DataDescriptor(
          RegisterInfo::FundamentalType::numeric, // fundamentalType
          false,                                  // isIntegral
          true,                                   // isSigned
          3 + 45,                                 // nDigits
      ...
      throw logic_error(
          "Wrong data width for data type IEEE754. Check your map file!");
    }
  } else if (dataType == FIXED_POINT) {
    if (width > 1) { // numeric type
      DataType rawDataInfo =
          (is2DMultiplexed_ ? DataType::none : DataType::int32);

      if (nFractionalBits_ > 0) {
        size_t nDigits = std::ceil(std::log10(std::pow(2, width_))) +
                         (signedFlag_ ? 1 : 0) +
                         (nFractionalBits_ != 0 ? 1 : 0);
        size_t nFractionalDigits =
            std::ceil(std::log10(std::pow(2, nFractionalBits_)));
