...
void DMapFilesParser::parse_file(const std::string &fileName) {
  ...
  std::string absolutePathToDMapDir =
      utilities::convertToAbsolutePath(utilities::extractDirectory(fileName));
  cleanAll();
  ...
  for (dmap_elem_iter = dmap->_deviceInfoElements.begin();
    ...
    std::string absPathToCurrentMapFile = utilities::concatenatePaths(
        absolutePathToDMapDir, dmap_elem_iter->mapFileName);
    map_file_iter =
        std::find_if(_mapFiles.begin(), _mapFiles.end(),
...
void DMapFilesParser::parse_one_directory(const std::string &dir) {
  ...
  while ((dirp = readdir(dp)) != NULL) {
    ...
    if (file_name.substr(found) == ".dmap") {
      ...
      for (dmap_elem_iter = dmap->_deviceInfoElements.begin();
           dmap_elem_iter != dmap->_deviceInfoElements.end();
           ++dmap_elem_iter) {
        map_file_iter =
            std::find_if(_mapFiles.begin(), _mapFiles.end(),
                         findMapFileByName_pred((*dmap_elem_iter).mapFileName));
        if (map_file_iter == _mapFiles.end()) {
          try {
            if ((*dmap_elem_iter).mapFileName[0] == '.') {
              map =
                  _mapFileParser.parse(dir_new + (*dmap_elem_iter).mapFileName);
            } else {
...
bool DMapFilesParser::check(
    DeviceInfoMap::ErrorList::ErrorElem::TYPE /*dlevel*/,
    RegisterInfoMap::ErrorList::ErrorElem::TYPE mlevel,
    DeviceInfoMap::ErrorList &dmap_err, RegisterInfoMap::ErrorList &map_err) {
  std::vector<std::pair<DeviceInfoMap::DeviceInfo, RegisterInfoMapPointer>>
      dmaps = _dmapElements;
  std::vector<std::pair<DeviceInfoMap::DeviceInfo,
                        RegisterInfoMapPointer>>::iterator iter_p,
      iter_n;
  bool ret = true;
  ...
  while (1) {
    if ((*iter_p).first.deviceName == (*iter_n).first.deviceName) {
      if ((*iter_p).first.uri != (*iter_n).first.uri ||
          (*iter_p).first.mapFileName != (*iter_n).first.mapFileName) {
        dmap_err.insert(DeviceInfoMap::ErrorList::ErrorElem(
            DeviceInfoMap::ErrorList::ErrorElem::ERROR,
            DeviceInfoMap::ErrorList::ErrorElem::NONUNIQUE_DEVICE_NAME,
            (*iter_p).first, (*iter_n).first));
        ret = false;
      }
    }
    iter_p = iter_n;
...
RegisterInfoMapPointer
DMapFilesParser::getMapFile(const std::string &dev_name) {
  std::vector<std::pair<DeviceInfoMap::DeviceInfo,
                        RegisterInfoMapPointer>>::iterator dmap_iter;
  dmap_iter = std::find_if(_dmapElements.begin(), _dmapElements.end(),
                           findDevInPairByName_pred(dev_name));
...
void DMapFilesParser::getdMapFileElem(const std::string &devName,
                                      DeviceInfoMap::DeviceInfo &dMapFileElem) {
  dMapFileElem = getdMapFileElem(devName);
}

DeviceInfoMap::DeviceInfo const &
DMapFilesParser::getdMapFileElem(const std::string &devName) {
  std::vector<std::pair<DeviceInfoMap::DeviceInfo,
                        RegisterInfoMapPointer>>::iterator dmap_iter;
  dmap_iter = std::find_if(_dmapElements.begin(), _dmapElements.end(),
                           findDevInPairByName_pred(devName));
  if (dmap_iter == _dmapElements.end()) {
...
void DMapFilesParser::getdMapFileElem(int elem_nr,
                                      DeviceInfoMap::DeviceInfo &dMapFileElem) {
  try {
...
std::ostream &operator<<(std::ostream &os, const DMapFilesParser &dmfp) {
  std::vector<std::pair<DeviceInfoMap::DeviceInfo,
                        RegisterInfoMapPointer>>::const_iterator iter;
  for (iter = dmfp._dmapElements.begin(); iter != dmfp._dmapElements.end();
       ++iter) {
