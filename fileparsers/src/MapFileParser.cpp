...
RegisterInfoMapPointer MapFileParser::parse(const std::string &file_name) {
  ...
  while (std::getline(file, line)) {
    ...
    if (line[0] == '@') {
      ...
      is >> md.name;
      if (!is) {
        throw ChimeraTK::logic_error("Parsing error in map file '" + file_name +
                                     "' on line " + std::to_string(line_nr));
      }
    ...
    std::pair<std::string, std::string> moduleAndNamePair =
        splitStringAtLastDot(moduleAndRegisterName);
    ...
    is >> std::setbase(0) >> nElements >> std::setbase(0) >> address >>
        std::setbase(0) >> nBytes;
    if (!is) {
      throw ChimeraTK::logic_error("Parsing error in map file '" + file_name +
                                   "' on line " + std::to_string(line_nr));
    }
    ...
    if (!failed) {
      ...
      if (is.fail()) {
        ...
        if (width > 32) {
          throw ChimeraTK::logic_error("Parsing error in map file '" +
                                       file_name + "' on line " +
    ...
    if (!failed) {
      ...
      if (is.fail()) {
        ...
        auto type_and_nFractionBits =
            getTypeAndNFractionalBits(bitInterpretation);
    ...
    RegisterInfoMap::RegisterInfo registerInfo(
        name, nElements, address, nBytes, bar, width, nFractionalBits,
  ...
  for (auto &info : *pmap) {
    ...
    if (info.name.substr(0, MULTIPLEXED_SEQUENCE_PREFIX.length()) !=
    ...
    size_t nChannels = 0;
    size_t nBytesPerEntry =
        0; // nb. of bytes per entry for all channels together
    ...
    while (cat.hasRegister(
        RegisterPath(info.module) /
        (SEQUENCE_PREFIX + name + "_" + std::to_string(nChannels)))) {
    ...
    // add it to the map
    RegisterInfoMap::RegisterInfo newEntry(
        name, nElements, info.address, info.nBytes, info.bar, maxWidth,
        (isInteger ? 0 : 9999) /*fractional bits*/, isSigned, info.module,
        nChannels, true, info.registerAccess);
...
MapFileParser::splitStringAtLastDot(std::string moduleDotName) {
...
std::pair<RegisterInfoMap::RegisterInfo::Type, int>
MapFileParser::getTypeAndNFractionalBits(std::string bitInterpretation) {
  if (bitInterpretation == "IEEE754") {
    ...
    try {
      ...
      throw ChimeraTK::logic_error(
          std::string("Map file error in bitInterpretation: wrong argument '") +
