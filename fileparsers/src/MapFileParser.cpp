...
namespace ChimeraTK {
  ...
  RegisterInfoMapPointer MapFileParser::parse(const std::string& file_name) {
    ...
    for(auto& info : *pmap) {
      ...
      // We have to aggregate the fractional/ signed information of all cannels. Afterwards we set fractional to 9999
      // (way out of range, max allowed is 1023) if there are fractional bits, just to indicate that the register is not
      // integer and probably a floating point accessor should be used (e.g. in QtHardMon).
      bool isSigned = false;
      bool isInteger = true;
      uint32_t maxWidth = 0;
