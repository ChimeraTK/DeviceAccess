 ...
 *  g++ read_temperature_doocs_zmq.cpp -o read_temperature_doocs_zmq `ChimeraTK-DeviceAccess-config --cppflags --ldflags` -lpthread -Wl,--no-as-needed -lChimeraTK-DeviceAccess-DoocsBackend
 */
# include <iostream>
...
int main (){
  ...
  // will receive anything. You can first create all accessors and then activate all of them at the same point in time with this.
  d.activateAsyncRead();

  // The third argument is a list of ChimeraTK::AccessMode. We only have one argument in it: AccessMode::wait_for_new_data.
  auto temperature =
    d.getScalarRegisterAccessor < float >( "Oven.temperatureReadback", 0 , {ChimeraTK::AccessMode::wait_for_new_data} );

  // The read now blocks until data is received. You can use it to syncrchonise your code to incoming data. No need for additional sleeps etc.
  while (true){
    temperature.read ();
