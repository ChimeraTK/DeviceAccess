...
namespace ChimeraTK::proxies {
  ...
  namespace instantiator {
    void instantiateMakeCapsule(DataType dt, std::byte* raw) {
      callForTypeNoVoid(dt, [&]([[maybe_unused]] auto dummy) {
        using UserType = decltype(dummy);
        ChimeraTK::NumericAddressedRegisterInfo info;
        auto cap = RawConverterCapsule<UserType>::makeCapsule(info, 0);
        cap->toCooked(raw);
        cap->toRaw({}, raw);
      });
    }
