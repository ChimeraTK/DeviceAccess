...
status(busy) flag turns back off. It then reads the data from the "readResponse" register.\n
   ...
   * For compatibility with the 3reg-mode, 'data' can be used instead of 'readData', and 'status' instead of 'busy'
   *
  ...
  class SubdeviceBackend : public DeviceBackendImpl {
   ...
   protected:
    ...
    static std::mutex _mutexMapMutex; // a mutex to proptext insertions into the map with mutexes.
