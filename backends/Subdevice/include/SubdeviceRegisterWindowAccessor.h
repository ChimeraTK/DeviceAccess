...
namespace ChimeraTK {
  ...
  class SubdeviceRegisterWindowAccessor : public NDRegisterAccessor<RegisterRawType> {
   ...
   protected:
    ...
    ScalarRegisterAccessor<uint64_t> _accChipSelect;       // chip select register, if present
    ScalarRegisterAccessor<uint64_t> _accAddress;          // address register
    OneDRegisterAccessor<ReadWriteDataType> _accWriteData; // write data register
    ScalarRegisterAccessor<ChimeraTK::Boolean> _accBusy;   // busy register, if present
    VoidRegisterAccessor _accReadRequest;                  // read request register, if present
    OneDRegisterAccessor<ReadWriteDataType> _accReadData;  // read data register

    // values needed in all transfers
