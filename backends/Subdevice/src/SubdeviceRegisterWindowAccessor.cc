...
namespace ChimeraTK {
  ...
  : NDRegisterAccessor<RegisterRawType>(registerPathName, {AccessMode::raw}), _backend(std::move(backend)),
    ...
    if(!parameters["chipSelectRegister"].empty()) {
      _accChipSelect = targetDevice->getRegisterAccessor<uint64_t>(parameters["chipSelectRegister"], 1, 0, {});
    }
    _accAddress = targetDevice->getRegisterAccessor<uint64_t>(parameters["address"], 1, 0, {});
    if(!parameters["writeData"].empty()) {
      _accWriteData = targetDevice->getRegisterAccessor<ReadWriteDataType>(parameters["writeData"], 0, 0, {});
    }
    else if(!parameters["data"].empty()) {
      _accWriteData = targetDevice->getRegisterAccessor<ReadWriteDataType>(parameters["data"], 0, 0, {});
    }

    if(!parameters["busy"].empty()) {
      _accBusy = targetDevice->getRegisterAccessor<ChimeraTK::Boolean>(parameters["busy"], 1, 0, {});
    }
    else if(!parameters["status"].empty()) {
      _accBusy = targetDevice->getRegisterAccessor<ChimeraTK::Boolean>(parameters["status"], 1, 0, {});
    }

    if(!parameters["readData"].empty()) {
      _accReadRequest = targetDevice->getRegisterAccessor<ChimeraTK::Void>(parameters["readRequest"], 1, 0, {});
      _accReadData = targetDevice->getRegisterAccessor<ReadWriteDataType>(parameters["readData"], 0, 0, {});
    }
    ...
    if(_accReadData && _accWriteData && _accReadData->getNumberOfSamples() != _accWriteData->getNumberOfSamples()) {
    ...
    if(_accWriteData) {
      _transferSize = sizeof(ReadWriteDataType) * _accWriteData->getNumberOfSamples();
    }
    else {
      assert(_accReadData);
      _transferSize = sizeof(ReadWriteDataType) * _accReadData->getNumberOfSamples();
    }
    ...
    try {
      ...
      for(size_t adr = _starTransferAddress; adr < _endTransferAddress; ++adr) {
        ...
        if(direction == TransferDirection::write) {
          auto* accessorStart = std::bit_cast<std::byte*>(_accWriteData->accessChannel(0).data());
        ...
        if(direction == TransferDirection::read) {
          _accReadData->read();
          auto* accessorStart = std::bit_cast<std::byte*>(_accReadData->accessChannel(0).data());
  ...
  std::list<boost::shared_ptr<TransferElement>> SubdeviceRegisterWindowAccessor<RegisterRawType,
    ...
    std::list<boost::shared_ptr<TransferElement>> retval = {_accAddress}; // always there

    if(_accWriteData) {
      retval.emplace_back(_accWriteData);
    }
    if(_accBusy) { // nullprt for 2reg
      retval.emplace_back(_accBusy);
    }
    if(_accChipSelect) {
