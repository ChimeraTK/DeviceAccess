...
PcieBackend::PcieBackend(std::string deviceNodeName, std::string mapFileName)
    : NumericAddressedBackend(mapFileName), _deviceID(0), _ioctlPhysicalSlot(0),
      _ioctlDriverVersion(0), _ioctlDMA(0), _deviceNodeName(deviceNodeName) {}
...
void PcieBackend::open() {
  ...
  if (_deviceID < 0) {
    throw ChimeraTK::runtime_error(
        createErrorStringWithErrnoText("Cannot open device: "));
  }
...
void PcieBackend::determineDriverAndConfigureIoctl() {
  ...
  if (ioctl(_deviceID, PCIEUNI_PHYSICAL_SLOT, &ioctlData) >= 0) {
    ...
    _readFunction = boost::bind(&PcieBackend::directRead, this, _1, _2, _3,
                                sizeof(int32_t));
    _readFunction = boost::bind(&PcieBackend::directRead, this, _1, _2, _3, _4);
    return;
  }
  ...
  ::close(_deviceID);
  throw ChimeraTK::runtime_error("Unsupported driver in device" +
                                 _deviceNodeName);
}
...
void PcieBackend::directRead(uint8_t bar, uint32_t address, int32_t *data,
                             size_t sizeInBytes) {
  if (_opened == false) {
    throw ChimeraTK::logic_error("Device closed");
  }
...
void PcieBackend::writeInternal(uint8_t bar, uint32_t address,
                                int32_t const *data) {
  device_rw l_RW;
  if (_opened == false) {
    throw ChimeraTK::logic_error("Device closed");
...
void PcieBackend::directWrite(uint8_t bar, uint32_t address,
...
void PcieBackend::readWithStruct(uint8_t bar, uint32_t address, int32_t *data,
                                 size_t sizeInBytes) {
  if (_opened == false) {
    throw ChimeraTK::logic_error("Device closed");
  }
...
void PcieBackend::read(uint8_t bar, uint32_t address, int32_t *data,
                       size_t sizeInBytes) {
  if (bar != 0xD) {
...
void PcieBackend::writeWithStruct(uint8_t bar, uint32_t address,
                                  int32_t const *data, size_t sizeInBytes) {
  if (_opened == false) {
    throw ChimeraTK::logic_error("Device closed");
  }
...
void PcieBackend::write(uint8_t bar, uint32_t address, int32_t const *data,
                        size_t sizeInBytes) {
  _writeFunction(bar, address, data, sizeInBytes);
}

void PcieBackend::readDMAViaStruct(uint8_t /*bar*/, uint32_t address,
                                   int32_t *data, size_t sizeInBytes) {
  ssize_t ret;
  device_rw l_RW;
  device_rw *pl_RW;
...
void PcieBackend::readDMAViaIoctl(uint8_t /*bar*/, uint32_t address,
                                  int32_t *data, size_t sizeInBytes) {
  if (_opened == false) {
    throw ChimeraTK::logic_error("Device closed");
  }
...
std::string PcieBackend::readDeviceInfo() {
  ...
  if (ioctl(_deviceID, _ioctlPhysicalSlot, &ioctlData) < 0) {
    throw ChimeraTK::runtime_error(
        createErrorStringWithErrnoText("Cannot read device info: "));
  }
  ...
  os << " DRV VER: "
     << (float)(ioctlData.offset / 10.0) + (float)ioctlData.data;
  return os.str();
}

std::string
PcieBackend::createErrorStringWithErrnoText(std::string const &startText) {
...
PcieBackend::createInstance(std::string address,
  ...
  return boost::shared_ptr<DeviceBackend>(
      new PcieBackend("/dev/" + address, parameters["map"]));
}
