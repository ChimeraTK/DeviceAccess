...
protected:
   ...
   * plugin */
  std::map<std::string, boost::function<boost::shared_ptr<DeviceBackend>(
                            std::string address,
  ...
  createBackendInternal(const DeviceInfoMap::DeviceInfo &deviceInfo);
  ...
  static boost::shared_ptr<DeviceBackend> failedRegistrationThrowerFunction(
      std::string host, std::string instance, std::list<std::string> parameters,
      std::string mapFileName, std::string exception_what);
