...
static const std::string MULTIPLEXED_SEQUENCE_PREFIX =
...
public:
  NumericAddressedBackendMuxedRegisterAccessor(
      const RegisterPath &registerPathName, size_t numberOfElements,
  ...
  bool mayReplaceOther(
    ...
    auto rhsCasted = boost::dynamic_pointer_cast<
        const NumericAddressedBackendMuxedRegisterAccessor<UserType>>(other);
    if (!rhsCasted)
      return false;
    if (_ioDevice != rhsCasted->_ioDevice)
      return false;
    if (_bar != rhsCasted->_bar)
      return false;
    if (_address != rhsCasted->_address)
      return false;
    if (_nBytes != rhsCasted->_nBytes)
      return false;
    if (_numberOfElements != rhsCasted->_numberOfElements)
      return false;
    if (_elementsOffset != rhsCasted->_elementsOffset)
      return false;
    if (_converters != rhsCasted->_converters)
      return false;
    return true;
  }
...
template <class UserType>
NumericAddressedBackendMuxedRegisterAccessor<UserType>::
    NumericAddressedBackendMuxedRegisterAccessor(
        const RegisterPath &registerPathName, size_t numberOfElements,
        size_t elementsOffset, boost::shared_ptr<DeviceBackend> _backend)
    : SyncNDRegisterAccessor<UserType>(registerPathName),
      _ioDevice(boost::dynamic_pointer_cast<NumericAddressedBackend>(_backend)),
      _registerPathName(registerPathName), _numberOfElements(numberOfElements),
  ...
  try {
    ...
    auto moduleAndRegister = MapFileParser::splitStringAtLastDot(
        _registerPathName.getWithAltSeparator());
    ...
    while (true) {
      ...
      _converters.push_back(FixedPointConverter(
          registerPathName, sequenceInfo.width, sequenceInfo.nFractionalBits,
          sequenceInfo.signedFlag));
    }

    // check if no sequences were found
    if (_converters.empty()) {
      throw ChimeraTK::logic_error("No sequences found for name \"" +
                                   _registerName + "\".");
    }
    ...
    if (_elementsOffset >= _nBlocks) {
      throw ChimeraTK::logic_error(
          "Requested element offset exceeds the size of the register!");
    }
    ...
    if (_numberOfElements + _elementsOffset > _nBlocks) {
      throw ChimeraTK::logic_error(
          "Requested number of elements exceeds the size of the register!");
    }
    ...
    if (_nBytes % sizeof(int32_t) > 0)
      _nBytes +=
          4 - _nBytes % sizeof(int32_t); // round up to the next multiple of 4
...
void NumericAddressedBackendMuxedRegisterAccessor<UserType>::doPostRead() {
  ...
  for (size_t blockIndex = 0; blockIndex < _nBlocks; ++blockIndex) {
    for (size_t sequenceIndex = 0; sequenceIndex < _converters.size();
         ++sequenceIndex) {
...
template <class UserType>
bool NumericAddressedBackendMuxedRegisterAccessor<UserType>::doWriteTransfer(
  ...
  for (size_t blockIndex = 0; blockIndex < _nBlocks; ++blockIndex) {
    for (size_t sequenceIndex = 0; sequenceIndex < _converters.size();
      ...
      case 2: // 16 bit variables
        *(reinterpret_cast<uint16_t *>(standOfMyioBuffer)) =
            _converters[sequenceIndex].toRaw(
                NDRegisterAccessor<UserType>::buffer_2D[sequenceIndex]
                                                       [blockIndex]);
        standOfMyioBuffer = standOfMyioBuffer + 2;
        break;
