...
class NumericAddressedBackendRegisterAccessor
    : public SyncNDRegisterAccessor<UserType> {
public:
  NumericAddressedBackendRegisterAccessor(boost::shared_ptr<DeviceBackend> dev,
      ...
      : SyncNDRegisterAccessor<UserType>(registerPathName),
        _dataConverter(registerPathName), isRaw(false),
    ...
    try {
      ...
      boost::shared_ptr<RegisterInfo> info =
          _dev->getRegisterInfo(registerPathName);
      _registerInfo =
          boost::static_pointer_cast<RegisterInfoMap::RegisterInfo>(info);
      _bar = _registerInfo->bar;
      _startAddress =
          _registerInfo->address + wordOffsetInRegister * sizeof(int32_t);

      // check number of words
      if (_numberOfWords == 0) {
        _numberOfWords =
            _registerInfo->getNumberOfElements() - wordOffsetInRegister;
      }
      if (_numberOfWords + wordOffsetInRegister >
          _registerInfo->getNumberOfElements()) {
        throw ChimeraTK::logic_error(
            "Requested number of words exceeds the size of the register '" +
            _registerPathName + "'!");
      }
      if (wordOffsetInRegister >= _registerInfo->getNumberOfElements()) {
        throw ChimeraTK::logic_error(
            "Requested offset exceeds the size of the register'" +
            _registerPathName + "'!");
      }
      ...
      _rawAccessor.reset(new NumericAddressedLowLevelTransferElement(
          _dev, _bar, _startAddress, _numberOfWords));
      ...
      _dataConverter =
          detail::createDataConverter<DataConverterType>(_registerInfo);

      if (flags.has(AccessMode::raw)) {
        if (typeid(UserType) != typeid(int32_t)) {
          throw ChimeraTK::logic_error(
              "Given UserType when obtaining the "
  ...
  doWriteTransfer(ChimeraTK::VersionNumber /*versionNumber*/ = {}) override {
    if (TransferElement::isInTransferGroup) {
      throw ChimeraTK::logic_error(
          "Calling read() or write() on an accessor which is part of a "
  ...
  void doPostRead() override {
    ...
    for (auto itdst = NDRegisterAccessor<UserType>::buffer_2D[0].begin();
         itdst != NDRegisterAccessor<UserType>::buffer_2D[0].end(); ++itdst) {
  ...
  bool mayReplaceOther(
    ...
    auto rhsCasted = boost::dynamic_pointer_cast<
        const NumericAddressedBackendRegisterAccessor<UserType,
                                                      DataConverterType>>(
        other);
    if (!rhsCasted)
      return false;
    if (_dev != rhsCasted->_dev)
      return false;
    if (_bar != rhsCasted->_bar)
      return false;
    if (_startAddress != rhsCasted->_startAddress)
      return false;
    if (_numberOfWords != rhsCasted->_numberOfWords)
      return false;
    if (isRaw != rhsCasted->isRaw)
      return false;
    if (_dataConverter != rhsCasted->_dataConverter)
      return false;
    return true;
  }
  ...
  bool isReadable() const override {
    return (_registerInfo->registerAccess &
            RegisterInfoMap::RegisterInfo::Access::READ) != 0;
  }

  bool isWriteable() const override {
    return (_registerInfo->registerAccess &
            RegisterInfoMap::RegisterInfo::Access::WRITE) != 0;
  }
  ...
  typedef NumericAddressedBackendRegisterAccessor<UserType, DataConverterType>
      THIS_TYPE;

  DEFINE_VIRTUAL_FUNCTION_TEMPLATE_VTABLE_FILLER(THIS_TYPE, getAsCooked_impl,
                                                 2);
  DEFINE_VIRTUAL_FUNCTION_TEMPLATE_VTABLE_FILLER(THIS_TYPE, setAsCooked_impl,
                                                 3);
...
protected:
  ...
  void replaceTransferElement(
    ...
    auto casted =
        boost::dynamic_pointer_cast<NumericAddressedLowLevelTransferElement>(
            newElement);
    if (casted && casted->isMergeable(_rawAccessor)) {
      size_t newStartAddress =
          std::min(casted->_startAddress, _rawAccessor->_startAddress);
      size_t newStopAddress =
          std::max(casted->_startAddress + casted->_numberOfBytes,
                   _rawAccessor->_startAddress + _rawAccessor->_numberOfBytes);
      size_t newNumberOfWords =
          (newStopAddress - newStartAddress) / sizeof(int32_t);
  ...
  struct dataConverterTemplateSpecialisationHelper {
    static CookedT toCooked(DataConverterType &, RawT &) {
      throw ChimeraTK::logic_error(
          "Getting as cooked is only available for raw accessors!");
    }
    static RawT toRaw(DataConverterType &, CookedT &) {
      throw ChimeraTK::logic_error(
          "Seting as cooked is only available for raw accessors!");
    }
  };
  template <typename CookedT>
  struct dataConverterTemplateSpecialisationHelper<int32_t, CookedT> {
    static CookedT toCooked(DataConverterType &dataConverter,
                            int32_t &rawValue) {
...
template <typename UserType, typename DataConverterType>
template <typename COOKED_TYPE>
COOKED_TYPE NumericAddressedBackendRegisterAccessor<
    UserType, DataConverterType>::getAsCooked_impl(unsigned int channel,
                                                   unsigned int sample) {
  if (isRaw) {
    return dataConverterTemplateSpecialisationHelper<UserType, COOKED_TYPE>::
        toCooked(_dataConverter,
...
template <typename UserType, typename DataConverterType>
template <typename COOKED_TYPE>
void NumericAddressedBackendRegisterAccessor<
    UserType, DataConverterType>::setAsCooked_impl(unsigned int channel,
                                                   unsigned int sample,
                                                   COOKED_TYPE value) {
  if (isRaw) {
    NDRegisterAccessor<UserType>::buffer_2D[channel][sample] =
        dataConverterTemplateSpecialisationHelper<UserType, COOKED_TYPE>::toRaw(
            _dataConverter, value);
  } else {
    throw ChimeraTK::logic_error(
        "Setting as cooked is only available for raw accessors!");
  }
}
...
template <>
void NumericAddressedBackendRegisterAccessor<int32_t,
                                             FixedPointConverter>::doPostRead();

template <>
void NumericAddressedBackendRegisterAccessor<int32_t,
                                             FixedPointConverter>::doPreWrite();

template <>
void NumericAddressedBackendRegisterAccessor<
    int32_t, FixedPointConverter>::doPostWrite();

DECLARE_MULTI_TEMPLATE_FOR_CHIMERATK_USER_TYPES(
    NumericAddressedBackendRegisterAccessor, FixedPointConverter);
DECLARE_MULTI_TEMPLATE_FOR_CHIMERATK_USER_TYPES(
    NumericAddressedBackendRegisterAccessor, IEEE754_SingleConverter);
