...
namespace ChimeraTK {
   ...
   public:
      ...
      try {
        ...
        if(flags.has(AccessMode::raw)) {
          if(typeid(UserType) != typeid(int32_t)) {
            throw ChimeraTK::logic_error(
                "Given UserType when obtaining the NumericAddressedBackendRegisterAccessor in raw mode does not "
    ...
    bool doWriteTransfer(ChimeraTK::VersionNumber /*versionNumber*/ = {}) override {
      if(TransferElement::isInTransferGroup) {
        throw ChimeraTK::logic_error(
            "Calling read() or write() on an accessor which is part of a TransferGroup is not allowed "
            "(Register name: " +
            _registerPathName + "')");
      }
      _rawAccessor->write();
  ...
  template<typename UserType, typename DataConverterType>
  template<typename COOKED_TYPE>
  void NumericAddressedBackendRegisterAccessor<UserType, DataConverterType>::setAsCooked_impl(
