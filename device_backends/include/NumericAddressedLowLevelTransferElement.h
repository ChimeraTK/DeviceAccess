...
public:
  NumericAddressedLowLevelTransferElement(
      boost::shared_ptr<NumericAddressedBackend> dev, size_t bar,
      size_t startAddress, size_t numberOfWords)
  ...
  TransferFuture doReadTransferAsync() override { // LCOV_EXCL_LINE
    ...
    throw ChimeraTK::logic_error(
        "NumericAddressedLowLevelTransferElement::readAsync() is not "
   ...
   * accessors. */
  bool
  isMergeable(const boost::shared_ptr<TransferElement const> &other) const {
    if (!_dev->canMergeRequests())
      return false;

    // accessor type, device and bar must be the same
    auto rhsCasted = boost::dynamic_pointer_cast<
        const NumericAddressedLowLevelTransferElement>(other);
    if (!rhsCasted)
      return false;
    ...
    if (_startAddress + _numberOfBytes < rhsCasted->_startAddress)
      return false;
    if (_startAddress > rhsCasted->_startAddress + rhsCasted->_numberOfBytes)
      return false;
    return true;
  }

  bool mayReplaceOther(
      const boost::shared_ptr<TransferElement const> &) const override {
  ...
  std::vector<int32_t>::iterator begin(size_t addressInBar) {
    return rawDataBuffer.begin() +
           (addressInBar - _startAddress) / sizeof(int32_t);
  }
  ...
  makeCopyRegisterDecorator() override { // LCOV_EXCL_LINE
    throw ChimeraTK::logic_error(
        "NumericAddressedLowLevelTransferElement::makeCopyRegisterDecorator() "
...
protected:
  ...
  void setAddress(size_t startAddress, size_t numberOfWords) {
    ...
    // update the name
    _name = "NALLTE:" + std::to_string(_startAddress) + "+" +
            std::to_string(_numberOfBytes);
  }
