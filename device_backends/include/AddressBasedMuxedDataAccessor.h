...
namespace mtca4u{
   ...
   */
  template<class UserType>
  class AddressBasedMuxedDataAccessor : public MultiplexedDataAccessor<UserType> {
    public:
      /** Constructor to intialise the members.
       */
      AddressBasedMuxedDataAccessor( boost::shared_ptr< DeviceBackend > const & ioDevice,
       ...
       * Default destructor
       */
      virtual ~AddressBasedMuxedDataAccessor() {};
  ...
  template<class UserType>
  std::vector<UserType> & AddressBasedMuxedDataAccessor<UserType>::operator[](
      size_t sequenceIndex){
      return _sequences[sequenceIndex];
  }

  template<class UserType, class SequenceWordType>
  class FixedTypeMuxedDataAccessor : public AddressBasedMuxedDataAccessor<UserType>{
    public:
  ...
  template <class UserType>
  class MixedTypeMuxedDataAccessor : public AddressBasedMuxedDataAccessor<UserType>{
    public:
  ...
  // derrived class.
  template<class UserType>
  AddressBasedMuxedDataAccessor<UserType>::AddressBasedMuxedDataAccessor(
      boost::shared_ptr< DeviceBackend > const & ioDevice,
      std::vector< FixedPointConverter > const & converters ) :
  ...
  FixedTypeMuxedDataAccessor<UserType, SequenceWordType>::FixedTypeMuxedDataAccessor(
      ...
      SequenceInfo const & areaInfo,
      std::vector< FixedPointConverter > const & converters ) :
      AddressBasedMuxedDataAccessor<UserType>(ioDevice, converters),
      _ioBuffer(areaInfo.nBytes),
      _areaInfo(areaInfo){

      MultiplexedDataAccessor<UserType>::_nBlocks = areaInfo.nBytes / sizeof(SequenceWordType) / converters.size();
  ...
  void FixedTypeMuxedDataAccessor<UserType, SequenceWordType>::read() {
      ...
      // else the read DMA
      if (isDMAMemoryArea()) {
        MultiplexedDataAccessor<UserType>::_ioDevice->readDMA(_areaInfo.bar,
            _areaInfo.address, reinterpret_cast<int32_t*>(&(_ioBuffer[0])),
            _areaInfo.nBytes);
      } else {
        MultiplexedDataAccessor<UserType>::_ioDevice->read(_areaInfo.bar,
            _areaInfo.address, reinterpret_cast<int32_t*>(&(_ioBuffer[0])),
  ...
  void FixedTypeMuxedDataAccessor<UserType, SequenceWordType>::write() {
      ...
      if (isDMAMemoryArea()) {
        ...
        MultiplexedDataAccessor<UserType>::_ioDevice->write(_areaInfo.bar,
            _areaInfo.address, reinterpret_cast<int32_t*>(&(_ioBuffer[0])),
            _areaInfo.nBytes);
      }
  }
  ...
  MixedTypeMuxedDataAccessor<UserType>::MixedTypeMuxedDataAccessor(boost::shared_ptr< DeviceBackend > const & ioDevice,
      ...
      MultiplexedDataAccessor<UserType>::_nBlocks = areaInfo.nBytes / 4 / _sizeOneBlock ;
  ...
  MultiplexedDataAccessor<UserType>::createInstance(
      ...
      while(true){
        ...
        if(converters.size()==1){
          sequenceWordSize=sequenceInfo.nBytes;
        }else{
          if(sequenceWordSize != sequenceInfo.nBytes){
            useFixedType=false;
          }
        }
      }
