...
public:
  ...
  getRegisterAccessor(const RegisterPath &registerPathName,
                      size_t numberOfWords, size_t wordOffsetInRegister,
                      AccessModeFlags flags);
  DEFINE_VIRTUAL_FUNCTION_TEMPLATE_VTABLE(
      getRegisterAccessor_impl,
      boost::shared_ptr<NDRegisterAccessor<T>>(const RegisterPath &, size_t,
                                               size_t, AccessModeFlags));
...
template <typename UserType>
boost::shared_ptr<NDRegisterAccessor<UserType>>
DeviceBackend::getRegisterAccessor(const RegisterPath &registerPathName,
                                   size_t numberOfWords,
                                   size_t wordOffsetInRegister,
                                   AccessModeFlags flags) {
  return CALL_VIRTUAL_FUNCTION_TEMPLATE(getRegisterAccessor_impl, UserType,
...
DeviceBackend::getRegisterAccessor(const RegisterPath &registerPathName,
  ...
  if (!enforceRawAccess) {
    return CALL_VIRTUAL_FUNCTION_TEMPLATE(
        getRegisterAccessor_impl, UserType, registerPathName, numberOfWords,
