...
public:
  ...
  static uint64_t calculateVirtualAddress(uint32_t registerOffsetInBar,
                                          uint8_t bar);
...
protected:
  ...
  std::multimap<AddressRange, boost::function<void(void)>>
  ...
  findCallbackFunctionsForAddressRange(AddressRange addressRange);
  ...
  /// resynchronisation.
  void writeRegisterWithoutCallback(uint8_t bar, uint32_t address,
                                    int32_t data);
  ...
  template <typename T> friend class DummyRegisterAccessor;

  template <typename T> friend class DummyMultiplexedRegisterAccessor;

  static std::string
  convertPathRelativeToDmapToAbs(std::string const &mapfileName);
  ...
  static boost::shared_ptr<DeviceBackend>
  returnInstance(const std::string &instanceId, Args &&... arguments) {
    if (instanceId == "") {
