...
public:
  ...
  ChimeraTK::VersionNumber getVersionNumber() const override {
    return _target->getVersionNumber();
  }
...
class NDRegisterAccessorDecoratorImpl<UserType, UserType>
...
class NDRegisterAccessorDecorator
    : public detail::NDRegisterAccessorDecoratorImpl<UserType, TargetUserType> {
public:
  ...
  void setPersistentDataStorage(
      boost::shared_ptr<ChimeraTK::PersistentDataStorage> storage) override {
    _target->setPersistentDataStorage(storage);
  }

  void replaceTransferElement(
      boost::shared_ptr<ChimeraTK::TransferElement> newElement) override;
...
protected:
  ...
  using detail::NDRegisterAccessorDecoratorImpl<UserType,
                                                TargetUserType>::_target;
};
...
createCopyDecorator(boost::shared_ptr<ChimeraTK::NDRegisterAccessor<T>> target);
...
void ChimeraTK::NDRegisterAccessorDecorator<UserType, TargetUserType>::
    replaceTransferElement(
        boost::shared_ptr<ChimeraTK::TransferElement> newElement) {
  auto casted = boost::dynamic_pointer_cast<
      ChimeraTK::NDRegisterAccessor<TargetUserType>>(newElement);
