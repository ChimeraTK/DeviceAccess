...
template <typename T> class DummyRegisterElement {
public:
  DummyRegisterElement(FixedPointConverter *_fpc, int _nbytes,
                       uint32_t *_buffer)
...
template <typename T> class DummyRegisterSequence {
public:
  DummyRegisterSequence(FixedPointConverter *_fpc, int _nbytes, int _pitch,
                        uint32_t *_buffer)
  ...
  inline DummyRegisterElement<T> operator[](unsigned int sample) {
    ...
    return DummyRegisterElement<T>(
        fpcptr, nbytes, reinterpret_cast<uint32_t *>(basePtr + pitch * sample));
  }
...
class DummyRegisterAccessor : public proxies::DummyRegisterElement<T>,
                              public DummyRegisterAddressChecker {
public:
  ...
  DummyRegisterAccessor(DummyBackend *dev, std::string module, std::string name)
    ...
    fpc = FixedPointConverter(module + "/" + name, registerInfo.width,
...
protected:
  ...
  inline uint32_t *getElement(unsigned int index) {
    return reinterpret_cast<uint32_t *>(
        &(_dev->_barContents[registerInfo.bar]
                            [registerInfo.address / sizeof(uint32_t) + index]));
  }

  /// return a proxy object
  inline proxies::DummyRegisterElement<T> getProxy(int index) {
    return proxies::DummyRegisterElement<T>(&fpc, sizeof(uint32_t),
                                            getElement(index));
  }
...
public:
  ...
  DummyMultiplexedRegisterAccessor(DummyBackend *dev, std::string module,
                                   std::string name)
      : _dev(dev), pitch(0) {
    _dev->_registerMapping->getRegisterInfo(MULTIPLEXED_SEQUENCE_PREFIX + name,
                                            registerInfo, module);
    ...
    while (true) {
      ...
      try {
        _dev->_registerMapping->getRegisterInfo(sequenceNameStream.str(), elem,
                                                module);
      } catch (ChimeraTK::logic_error &) {
        break;
      }
      // create fixed point converter for sequence
      fpc.push_back(FixedPointConverter(module + "/" + name, elem.width,
                                        elem.nFractionalBits, elem.signedFlag));
  ...
  inline proxies::DummyRegisterSequence<T> operator[](unsigned int sequence) {
    int8_t *basePtr =
        reinterpret_cast<int8_t *>(_dev->_barContents[registerInfo.bar].data());
    uint32_t *seq = reinterpret_cast<uint32_t *>(basePtr + offsets[sequence]);
    return proxies::DummyRegisterSequence<T>(&(fpc[sequence]), nbytes[sequence],
                                             pitch, seq);
  }
