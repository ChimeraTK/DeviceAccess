...
template <typename T>
struct CopyRegisterDecorator : ChimeraTK::NDRegisterAccessorDecorator<T>,
                               CopyRegisterDecoratorTrait {
  CopyRegisterDecorator(
      const boost::shared_ptr<ChimeraTK::NDRegisterAccessor<T>> &target)
      : ChimeraTK::NDRegisterAccessorDecorator<T>(target) {
    if (!target->isReadable()) {
      throw ChimeraTK::logic_error(
          "ChimeraTK::CopyRegisterDecorator: Target accessor is not readable.");
    }
  }

  void doPreWrite() override {
    throw ChimeraTK::logic_error(
        "ChimeraTK::CopyRegisterDecorator: Accessor is not writeable.");
  }
