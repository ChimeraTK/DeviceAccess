 ...
 *  the actual IO. */
template <typename UserType> class NDRegisterAccessor : public TransferElement {
public:
  ...
  const UserType &accessData(unsigned int sample) const {
  ...
  DEFINE_VIRTUAL_FUNCTION_TEMPLATE_VTABLE(getAsCooked_impl,
                                          T const(unsigned int, unsigned int));
  template <typename COOKED_TYPE>
  COOKED_TYPE getAsCooked_impl(unsigned int channel, unsigned int sample) const;

  template <typename COOKED_TYPE>
  void setAsCooked(unsigned int channel, unsigned int sample,
                   COOKED_TYPE value);
  DEFINE_VIRTUAL_FUNCTION_TEMPLATE_VTABLE(setAsCooked_impl,
...
protected:
  ...
  DEFINE_VIRTUAL_FUNCTION_TEMPLATE_VTABLE_FILLER(NDRegisterAccessor<UserType>,
                                                 getAsCooked_impl, 2);
  DEFINE_VIRTUAL_FUNCTION_TEMPLATE_VTABLE_FILLER(NDRegisterAccessor<UserType>,
                                                 setAsCooked_impl, 3);
};
...
template <typename COOKED_TYPE>
COOKED_TYPE
NDRegisterAccessor<UserType>::getAsCooked(unsigned int channel,
                                          unsigned int sample) const {
  return CALL_VIRTUAL_FUNCTION_TEMPLATE(getAsCooked_impl, COOKED_TYPE, channel,
                                        sample);
}
...
template <typename COOKED_TYPE>
COOKED_TYPE
NDRegisterAccessor<UserType>::getAsCooked_impl(unsigned int /*channel*/,
                                               unsigned int /*sample*/) const {
  throw ChimeraTK::logic_error(
      "Reading as cooked is not available for this accessor");
}

template <typename UserType>
template <typename COOKED_TYPE>
void NDRegisterAccessor<UserType>::setAsCooked(unsigned int channel,
                                               unsigned int sample,
                                               COOKED_TYPE value) {
  CALL_VIRTUAL_FUNCTION_TEMPLATE(setAsCooked_impl, COOKED_TYPE, channel, sample,
                                 value);
}
...
void NDRegisterAccessor<UserType>::setAsCooked_impl(unsigned int /*channel*/,
                                                    ...
                                                    COOKED_TYPE /*value*/) {
  throw ChimeraTK::logic_error(
      "Setting as cooked is not available for this accessor");
}
