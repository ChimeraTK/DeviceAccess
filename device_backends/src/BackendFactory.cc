...
namespace ChimeraTK {
    ...
    creatorMap_compat[make_pair(backendType, "")] = [creatorFunction, sdmParameterNames](std::string,
      ...
      if(!mapFileName.empty()) {
        if(pars["map"].empty()) {
          ...
          std::cout << "WARNING: You have specified the map file name twice, in the parameter list and in the 3rd "
  ...
  boost::shared_ptr<DeviceBackend> BackendFactory::createBackendInternal(const DeviceInfoMap::DeviceInfo& deviceInfo) {
    ...
    if(Utilities::isSdm(deviceInfo.uri)) {
      ...
      // std::cout<<"# Using the device node in a dmap file is deprecated. Please change to sdm if
      // applicable."<<std::endl;
    }
#ifdef _DEBUG
