...
void BackendFactory::registerBackendType(
  ...
  if (creatorMap.find(backendType) != creatorMap.end()) {
    throw ChimeraTK::logic_error("A backend with the type name '" +
                                 backendType +
  ...
  if (version != CHIMERATK_DEVICEACCESS_VERSION) {
    ...
    errorMessage << "Backend plugin '" << backendType
                 << "' compiled with wrong DeviceAccess version " << version
                 << ". Please recompile with version "
                 << CHIMERATK_DEVICEACCESS_VERSION;
    creatorMap_compat[make_pair(backendType, "")] =
        boost::bind(BackendFactory::failedRegistrationThrowerFunction, _1, _2,
                    _3, _4, errorMessage.str());
    std::string errorMessageString = errorMessage.str();
    creatorMap[backendType] =
        [errorMessageString](std::string, std::map<std::string, std::string>)
  ...
  creatorMap_compat[make_pair(
      backendType, "")] = [creatorFunction,
                           sdmParameterNames](std::string, std::string instance,
                                              std::list<std::string> parameters,
                                              std::string mapFileName) {
    std::map<std::string, std::string> pars;
    size_t i = 0;
    for (auto &p : parameters) {
      if (i >= sdmParameterNames.size())
        break;
    ...
    if (!mapFileName.empty()) {
      if (pars["map"].empty()) {
        ...
        std::cout << "WARNING: You have specified the map file name twice, in "
                  ...
                  << std::endl;
        std::cout
            << "Please only specify the map file name in the parameter list!"
            << std::endl;
      }
    }
...
void BackendFactory::registerBackendType(
    std::string interface, std::string protocol,
    boost::shared_ptr<DeviceBackend> (*creatorFunction)(
  ...
  if (version != CHIMERATK_DEVICEACCESS_VERSION) {
    ...
    // should be usable if the particular backend is not used.
    std::stringstream errorMessage;
    errorMessage << "Backend plugin '"
                 << interface << "' compiled with wrong DeviceAccess version "
                 << version << ". Please recompile with version "
                 << CHIMERATK_DEVICEACCESS_VERSION;
...
BackendFactory::BackendFactory() {
  ...
  registerBackendType("rebot", &RebotBackend::createInstance,
                      {"ip", "port", "map"});
  registerBackendType("logicalNameMap",
                      &LogicalNameMappingBackend::createInstance, {"map"});
  registerBackendType("subdevice", &SubdeviceBackend::createInstance, {"map"});
  registerBackendType("sharedMemoryDummy", &SharedDummyBackend::createInstance,
                      {"map"});
}
/********************************************************************************************************************/
...
BackendFactory::createBackend(std::string aliasOrUri) {
  std::lock_guard<std::mutex> lock(_mutex);

  if (Utilities::isDeviceDescriptor(aliasOrUri) ||
      Utilities::isSdm(aliasOrUri)) {
...
boost::shared_ptr<DeviceBackend> BackendFactory::createBackendInternal(
  ...
  if (Utilities::isDeviceDescriptor(deviceInfo.uri)) {
    ...
    try {
      auto backend =
          (creatorMap.at(cdd.backendType))(cdd.address, cdd.parameters);
      _existingBackends[deviceInfo.uri] = backend;
      return backend;
    } catch (std::out_of_range &) {
      throw ChimeraTK::logic_error(
          "Unknown backend: \"" + cdd.backendType + "\" at " +
  ...
  for (std::list<std::string>::iterator it = sdm._Parameters.begin();
       it != sdm._Parameters.end(); ++it) {
    std::cout << *it << std::endl;
  }
#endif
  for (auto iter = creatorMap_compat.begin(); iter != creatorMap_compat.end();
       ++iter) {
#ifdef _DEBUG
    std::cout << "Pair:" << iter->first.first << "+" << iter->first.second
              << std::endl;
#endif
    if ((iter->first.first == sdm._Interface)) {
      auto backend = (iter->second)(sdm._Host, sdm._Instance, sdm._Parameters,
  ...
  throw ChimeraTK::logic_error("Unregistered device: Interface = " +
                               sdm._Interface + " Protocol = " + sdm._Protocol);
...
void BackendFactory::loadAllPluginsFromDMapFile() {
  ...
  for (auto lib : dmap->getPluginLibraries()) {
    try {
      ...
      std::cerr << "Error: Caught exception loading plugin '" << lib
                << "' specified in dmap file: " << e.what() << std::endl;
...
boost::shared_ptr<DeviceBackend>
BackendFactory::failedRegistrationThrowerFunction(
    std::string /*host*/, std::string /*instance*/,
    std::list<std::string> /*parameters*/, std::string /*mapFileName*/,
