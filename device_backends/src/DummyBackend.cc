...
#define TRY_REGISTER_ACCESS(COMMAND)                                           \
  try {                                                                        \
    ...
    errorMessage << "Invalid address offset " << address << " in bar "         \
                 << static_cast<int>(bar) << "."                               \
...
DummyBackend::DummyBackend(std::string mapFileName)
    : NumericAddressedBackend(mapFileName), _mapFile(mapFileName) {
...
DummyBackend::getBarSizesInBytesFromRegisterMapping() const {
  std::map<uint8_t, size_t> barSizesInBytes;
  for (RegisterInfoMap::const_iterator mappingElementIter =
           _registerMapping->begin();
...
void DummyBackend::writeRegisterWithoutCallback(uint8_t bar, uint32_t address,
                                                int32_t data) {
  std::lock_guard<std::mutex> lock(mutex);
  TRY_REGISTER_ACCESS(_barContents[bar].at(address / sizeof(int32_t)) = data;);
}

void DummyBackend::read(uint8_t bar, uint32_t address, int32_t *data,
                        size_t sizeInBytes) {
  std::lock_guard<std::mutex> lock(mutex);
  if (!_opened) {
  ...
  for (unsigned int wordIndex = 0; wordIndex < sizeInBytes / sizeof(int32_t);
       ++wordIndex) {
    TRY_REGISTER_ACCESS(data[wordIndex] =
                            _barContents[bar].at(wordBaseIndex + wordIndex););
  }
}

void DummyBackend::write(uint8_t bar, uint32_t address, int32_t const *data,
                         size_t sizeInBytes) {
  {
    std::lock_guard<std::mutex> lock(mutex);
    if (!_opened) {
    ...
    for (unsigned int wordIndex = 0; wordIndex < sizeInBytes / sizeof(int32_t);
      ...
      TRY_REGISTER_ACCESS(_barContents[bar].at(wordBaseIndex + wordIndex) =
                              data[wordIndex];);
    }
  }
  ...
  runWriteCallbackFunctionsForAddressRange(
      AddressRange(bar, address, sizeInBytes));
}

std::string DummyBackend::readDeviceInfo() {
  std::stringstream info;
  info << "DummyBackend with mapping file "
       << _registerMapping->getMapFileName();
  return info.str();
}

uint64_t DummyBackend::calculateVirtualAddress(uint32_t registerOffsetInBar,
                                               uint8_t bar) {
  return (static_cast<uint64_t>(bar & BAR_MASK)
          << BAR_POSITION_IN_VIRTUAL_REGISTER) |
...
void DummyBackend::setReadOnly(uint8_t bar, uint32_t address,
                               size_t sizeInWords) {
  for (size_t i = 0; i < sizeInWords; ++i) {
    uint64_t virtualAddress =
        calculateVirtualAddress(address + i * sizeof(int32_t), bar);
    _readOnlyAddresses.insert(virtualAddress);
  }
}

void DummyBackend::setReadOnly(AddressRange addressRange) {
  setReadOnly(addressRange.bar, addressRange.offset,
              addressRange.sizeInBytes / sizeof(int32_t));
}
...
void DummyBackend::setWriteCallbackFunction(
  ...
  _writeCallbackFunctions.insert(
      std::pair<AddressRange, boost::function<void(void)>>(
          addressRange, writeCallbackFunction));
}

void DummyBackend::runWriteCallbackFunctionsForAddressRange(
    AddressRange addressRange) {
  std::list<boost::function<void(void)>> callbackFunctionsForThisRange =
      findCallbackFunctionsForAddressRange(addressRange);
  for (std::list<boost::function<void(void)>>::iterator functionIter =
           callbackFunctionsForThisRange.begin();
...
DummyBackend::findCallbackFunctionsForAddressRange(AddressRange addressRange) {
  ...
  AddressRange endAddress(addressRange.bar,
                          addressRange.offset + addressRange.sizeInBytes, 0);

  std::multimap<AddressRange, boost::function<void(void)>>::iterator
      startIterator = _writeCallbackFunctions.lower_bound(firstAddressInBar);

  std::multimap<AddressRange, boost::function<void(void)>>::iterator
      endIterator = _writeCallbackFunctions.lower_bound(endAddress);

  std::list<boost::function<void(void)>> returnList;
  for (std::multimap<AddressRange, boost::function<void(void)>>::iterator
           callbackIter = startIterator;
...
bool DummyBackend::isWriteRangeOverlap(AddressRange firstRange,
...
DummyBackend::createInstance(std::string address,
...
std::string
DummyBackend::convertPathRelativeToDmapToAbs(const std::string &mapfileName) {
  std::string dmapDir = parserUtilities::extractDirectory(
      BackendFactory::getInstance().getDMapFilePath());
  std::string absPathToDmapDir =
      parserUtilities::convertToAbsolutePath(dmapDir);
