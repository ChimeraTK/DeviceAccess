...
std::map< uint8_t, size_t > DummyBackend::getBarSizesInBytesFromRegisterMapping() const{
	...
	for (RegisterInfoMap::const_iterator mappingElementIter = _registerMapping->begin();
		...
		barSizesInBytes[mappingElementIter->_bar] =
				std::max( barSizesInBytes[mappingElementIter->_bar],
						static_cast<size_t> (mappingElementIter->_addressOffset +
								mappingElementIter->_size ) );
	}
