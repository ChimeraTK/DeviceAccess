...
namespace ChimeraTK {
  ...
  AsyncNDRegisterAccessor<UserType>::AsyncNDRegisterAccessor(const boost::shared_ptr<DeviceBackend>& backend,
      ...
      size_t nElements, AccessModeFlags accessModeFlags, AccessorInstanceDescriptor descriptor, std::string const& unit,
      std::string const& description)

  : NDRegisterAccessor<UserType>(name, accessModeFlags, unit, description), _backend(backend),
    _accessorManager(manager), _receiveBuffer(nChannels, nElements), _descriptor(descriptor) {
