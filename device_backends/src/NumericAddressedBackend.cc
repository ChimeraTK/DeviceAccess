...
NumericAddressedBackend::getRegisterInfo(const RegisterPath &registerPathName) {
  if (!registerPathName.startsWith(numeric_address::BAR)) {
    ...
    if (components.size() != 3) {
      throw ChimeraTK::logic_error("Illegal numeric address: '" +
                                   (registerPathName) + "'");
    }
    ...
    return boost::make_shared<RegisterInfoMap::RegisterInfo>(
        registerPathName, nElements, address, nBytes, bar);
  }
}

/********************************************************************************************************************/

void NumericAddressedBackend::read(const std::string &regModule,
                                   const std::string &regName, int32_t *data,
...
void NumericAddressedBackend::write(const std::string &regModule,
...
boost::shared_ptr<const RegisterInfoMap>
NumericAddressedBackend::getRegisterMap() const {
  return _registerMap;
}
...
NumericAddressedBackend::getRegistersInModule(
...
void NumericAddressedBackend::checkRegister(
    const std::string &regName, const std::string &regModule, size_t dataSize,
...
NumericAddressedBackend::getRegisterAccessor_impl(
  ...
  if (info->getNumberOfDimensions() <= 1) {
    if (registerInfo->dataType ==
        RegisterInfoMap::RegisterInfo::Type::FIXED_POINT) {
      accessor = boost::shared_ptr<NDRegisterAccessor<UserType>>(
          new NumericAddressedBackendRegisterAccessor<UserType,
                                                      FixedPointConverter>(
              shared_from_this(), registerPathName, numberOfWords,
              wordOffsetInRegister, flags));
    } else if (registerInfo->dataType ==
