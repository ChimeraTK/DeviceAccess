...
SubdeviceRegisterAccessor::SubdeviceRegisterAccessor(
    boost::shared_ptr<SubdeviceBackend> backend,
    ...
    boost::shared_ptr<NDRegisterAccessor<int32_t>> accStatus, size_t byteOffset,
    size_t numberOfWords)
    : SyncNDRegisterAccessor<int32_t>(registerPathName), _backend(backend),
      _accAddress(accAddress), _accData(accData), _accStatus(accStatus),
      _byteOffset(byteOffset), _numberOfWords(numberOfWords) {
...
bool SubdeviceRegisterAccessor::doWriteTransfer(ChimeraTK::VersionNumber) {
  ...
  size_t idx = 0;
  for (size_t adr = _byteOffset; adr < _byteOffset + 4 * _numberOfWords;
       adr += 4) {
...
bool SubdeviceRegisterAccessor::mayReplaceOther(
    const boost::shared_ptr<TransferElement const> &) const {
  return false;
}
...
SubdeviceRegisterAccessor::getHardwareAccessingElements() {
...
SubdeviceRegisterAccessor::getInternalElements() {
  return {_accAddress, _accData, _accStatus};
}

/*********************************************************************************************************************/

void SubdeviceRegisterAccessor::replaceTransferElement(
