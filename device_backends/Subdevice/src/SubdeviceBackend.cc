  ...
  if (!address.empty()) {
    if (parameters.size() > 1) {
      throw ChimeraTK::logic_error(
          "SubdeviceBackend: You cannot specify both the address string and "
          "parameters "
...
SubdeviceBackend::SubdeviceBackend(
  ...
  if (parameters["type"].empty()) {
    throw ChimeraTK::logic_error(
        "SubdeviceBackend: Type must be specified in the device descriptor.");
  }
  ...
  // type "3regs":
    ...
    if (!parameters["sleep"].empty()) {
      try {
        ...
        throw ChimeraTK::logic_error(
            "SubdeviceBackend: Invalid value for parameter 'sleep': '" +
            parameters["sleep"] + "': " + e.what());
      }
    }
    ...
    if (parameters["sleep"].empty()) {
      throw ChimeraTK::logic_error(
          "SubdeviceBackend: Target sleep time must be specified in the device "
          "descriptor for type '3regs'.");
    }
    targetAddress = parameters["address"];
    targetData = parameters["data"];
  ...
  // unknown type
    ...
    throw ChimeraTK::logic_error("SubdeviceBackend: Unknown type '" +
                                 parameters["type"] + "' specified.");
  }

  // parse map file
  if (parameters["map"].empty()) {
    throw ChimeraTK::logic_error(
        "SubdeviceBackend: Map file must be specified.");
  }
...
class FixedPointConvertingDecorator
    : public NDRegisterAccessorDecorator<UserType, TargetUserType> {
public:
  FixedPointConvertingDecorator(
      const boost::shared_ptr<ChimeraTK::NDRegisterAccessor<TargetUserType>>
          &target,
  ...
  void doPostRead() override {
    ...
    for (size_t i = 0; i < this->buffer_2D.size(); ++i) {
      for (size_t j = 0; j < this->buffer_2D[i].size(); ++j) {
        buffer_2D[i][j] = _fixedPointConverter.toCooked<UserType>(
  ...
  void doPreWrite() override {
    for (size_t i = 0; i < this->buffer_2D.size(); ++i) {
      for (size_t j = 0; j < this->buffer_2D[i].size(); ++j) {
        _target->accessChannel(i)[j] =
            _fixedPointConverter.toRaw<UserType>(buffer_2D[i][j]);
      }
    }
    _target->preWrite();
  }
  ...
  bool mayReplaceOther(const boost::shared_ptr<ChimeraTK::TransferElement const>
    ...
    auto casted = boost::dynamic_pointer_cast<
        FixedPointConvertingDecorator<UserType, TargetUserType> const>(other);
    if (!casted)
      return false;
...
class FixedPointConvertingRawDecorator
    : public NDRegisterAccessorDecorator<TargetUserType> {
public:
  FixedPointConvertingRawDecorator(
      const boost::shared_ptr<ChimeraTK::NDRegisterAccessor<TargetUserType>>
          &target,
      FixedPointConverter fixedPointConverter)
  ...
  COOKED_TYPE getAsCooked_impl(unsigned int channel, unsigned int sample) {
    return _fixedPointConverter.toCooked<COOKED_TYPE>(
        buffer_2D[channel][sample]);
  }

  template <typename COOKED_TYPE>
  void setAsCooked_impl(unsigned int channel, unsigned int sample,
                        COOKED_TYPE value) {
  ...
  bool mayReplaceOther(const boost::shared_ptr<ChimeraTK::TransferElement const>
                           &other) const override {
    auto casted = boost::dynamic_pointer_cast<
        FixedPointConvertingRawDecorator<TargetUserType> const>(other);
    if (!casted)
      return false;
    if (_fixedPointConverter != casted->_fixedPointConverter)
      return false;
...
SubdeviceBackend::getRegisterAccessor_impl(const RegisterPath &registerPathName,
                                           size_t numberOfWords,
                                           size_t wordOffsetInRegister,
                                           AccessModeFlags flags) {
  if (type == Type::area) {
    return getRegisterAccessor_area<UserType>(registerPathName, numberOfWords,
                                              wordOffsetInRegister, flags);
  } else if (type == Type::threeRegisters || type == Type::twoRegisters) {
    return getRegisterAccessor_3regs<UserType>(registerPathName, numberOfWords,
                                               wordOffsetInRegister, flags);
  }
...
template <typename UserType>
boost::shared_ptr<NDRegisterAccessor<UserType>>
SubdeviceBackend::getRegisterAccessor_area(const RegisterPath &registerPathName,
                                           size_t numberOfWords,
                                           size_t wordOffsetInRegister,
                                           AccessModeFlags flags) {
  ...
  auto info = boost::static_pointer_cast<RegisterInfoMap::RegisterInfo>(
      _catalogue.getRegister(registerPathName));

  // check that the bar is 0
  if (info->bar != 0) {
    throw ChimeraTK::logic_error(
        "SubdeviceBackend: BARs other then 0 are not supported. Register '" +
  ...
  if (info->is2DMultiplexed) {
    throw ChimeraTK::logic_error(
        "SubdeviceBackend: 2D multiplexed registers are not yet supported.");
  }
  ...
  if (numberOfWords == 0) {
    ...
    throw ChimeraTK::logic_error(
        "SubdeviceBackend: Requested " + std::to_string(numberOfWords) +
        " elements from register '" + registerPathName +
        "', which only has a length of " + std::to_string(info->nElements) +
        " elements.");
  }
  ...
  auto rawAcc = targetDevice->getRegisterAccessor<int32_t>(
      targetArea, numberOfWords, wordOffset, flags);
  ...
  if (!isRaw) {
    return boost::make_shared<FixedPointConvertingDecorator<UserType, int32_t>>(
        rawAcc, FixedPointConverter(registerPathName, info->width,
                                    info->nFractionalBits, info->signedFlag));
  } else {
    // this is handled by the template specialisation for int32_t
    throw ChimeraTK::logic_error(
        "Given UserType when obtaining the SubdeviceBackend in raw mode does "
        "not "
...
template <typename UserType>
boost::shared_ptr<NDRegisterAccessor<UserType>>
SubdeviceBackend::getRegisterAccessor_3regs(
  ...
  // obtain target accessor in raw mode
  auto accAddress = targetDevice->getRegisterAccessor<int32_t>(
      targetAddress, 1, 0, {AccessMode::raw});
  auto accData = targetDevice->getRegisterAccessor<int32_t>(targetData, 1, 0,
                                                            {AccessMode::raw});
  ...
  auto sharedThis =
      boost::enable_shared_from_this<DeviceBackend>::shared_from_this();
  auto rawAcc = boost::make_shared<SubdeviceRegisterAccessor>(
      boost::dynamic_pointer_cast<SubdeviceBackend>(sharedThis),
      registerPathName, accAddress, accData, accStatus, byteOffset,
      numberOfWords);
...
template <>
boost::shared_ptr<NDRegisterAccessor<int32_t>>
SubdeviceBackend::getRegisterAccessor_area<int32_t>(
    const RegisterPath &registerPathName, size_t numberOfWords,
    size_t wordOffsetInRegister, AccessModeFlags flags) {
  assert(type == Type::area);
...
template <>
boost::shared_ptr<NDRegisterAccessor<int32_t>>
SubdeviceBackend::getRegisterAccessor_3regs<int32_t>(
    const RegisterPath &registerPathName, size_t numberOfWords,
    size_t wordOffsetInRegister, AccessModeFlags flags) {
  assert(type == Type::threeRegisters || type == Type::twoRegisters);
  flags.checkForUnknownFlags({AccessMode::raw});
