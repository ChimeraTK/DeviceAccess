...
card_number_fallback() {
  echo $1 | / bin / sed 's:xdma\([0-9][0-9]*\)_events_\([0-9][0-9]*\):xdma/card\1/events\2:' |
      ...
      / bin / sed 's:xdma\([0-9][0-9]*\)_\([a-z]*\):xdma/card\1/\2:' exit 0
}

#Get PCI bus address(w / o device function) from device path
PCIADDR =
    $(echo $2 | sed - n - E 's#/devices/pci.*([0-9]{4}:[0-9]{2}:[0-9]{2})\.[0-9]/xdma/.*#\1#p') if[-z "${PCIADDR}"]; then
