...
public:
  LNMBackendVariableAccessor(boost::shared_ptr<DeviceBackend> dev,
  ...
  bool doWriteTransfer(ChimeraTK::VersionNumber versionNumber = {}) override {
    if (isReadOnly()) {
      throw ChimeraTK::logic_error(
          "Writing to constant-type registers of logical name mapping devices "
          "is not possible.");
    }
    for (size_t i = 0; i < NDRegisterAccessor<UserType>::buffer_2D[0].size();
         ++i) {
      auto &v = _info->value_int[i + _wordOffsetInRegister];
      v = _fixedPointConverter.toRaw(
  ...
  void doPostRead() override {
    for (size_t i = 0; i < NDRegisterAccessor<UserType>::buffer_2D[0].size();
      ...
      auto &v = _info->value_int[i + _wordOffsetInRegister];
      NDRegisterAccessor<UserType>::buffer_2D[0][i] =
          _fixedPointConverter.toCooked<UserType>(v);
    }
    currentVersion = {};
  }
