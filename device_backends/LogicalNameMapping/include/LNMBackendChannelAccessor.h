   ...
   public:
    ...
    : SyncNDRegisterAccessor<UserType>(registerPathName), _registerPathName(registerPathName) {
      try {
        ...
        if(_info.targetType != LNMBackendRegisterInfo::TargetType::CHANNEL) {
          throw ChimeraTK::logic_error("LNMBackendChannelAccessor used for wrong register type."); // LCOV_EXCL_LINE
                                                                                                   // (impossible to
                                                                                                   // test...)
        }
    ...
    bool doWriteTransfer(ChimeraTK::VersionNumber /*versionNumber*/ = {}) override {
      throw ChimeraTK::logic_error(
          "Writing to channel-type registers of logical name mapping devices is not supported.");
    }
