...
public:
  LNMBackendChannelAccessor(boost::shared_ptr<DeviceBackend> dev,
    ...
    try {
      ...
      if (flags.has(AccessMode::raw)) {
        throw ChimeraTK::logic_error(
            "LNMBackendChannelAccessor: raw access not yet supported!");
      }
      ...
      _accessor = targetDevice->getRegisterAccessor<UserType>(
          RegisterPath(_info.registerName), numberOfWords, wordOffsetInRegister,
          false);
      // verify channel number
      if (_info.channel >= _accessor->getNumberOfChannels()) {
        throw ChimeraTK::logic_error(
            "LNMBackendChannelAccessor: Requested channel number " +
      ...
      NDRegisterAccessor<UserType>::buffer_2D[0].resize(
          _accessor->getNumberOfSamples());
    } catch (...) {
      this->shutdown();
      throw;
    }
  ...
  void doPostRead() override {
    _accessor->postRead();
    _accessor->accessChannel(_info.channel)
        .swap(NDRegisterAccessor<UserType>::buffer_2D[0]);
  };

  bool mayReplaceOther(
      const boost::shared_ptr<TransferElement const> &other) const override {
    auto rhsCasted =
        boost::dynamic_pointer_cast<const LNMBackendChannelAccessor<UserType>>(
            other);
    if (!rhsCasted)
      return false;
    if (_registerPathName != rhsCasted->_registerPathName)
      return false;
...
protected:
  ...
  void replaceTransferElement(
    ...
    auto casted =
        boost::dynamic_pointer_cast<NDRegisterAccessor<UserType>>(newElement);
