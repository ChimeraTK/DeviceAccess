...
public:
  LNMBackendBitAccessor(boost::shared_ptr<DeviceBackend> dev,
    ...
    try {
      ...
      if (flags.has(AccessMode::raw)) {
        throw ChimeraTK::logic_error(
            "LNMBackendBitAccessor: raw access not supported!");
      }
      ...
      NDRegisterAccessor<UserType>::buffer_2D[0][0] =
          _fixedPointConverter.toCooked<UserType>(false);
      // set the bit mask
  ...
  void doPostRead() override {
    ...
    if (_accessor->accessData(0) & _bitMask) {
      NDRegisterAccessor<UserType>::buffer_2D[0][0] =
          _fixedPointConverter.toCooked<UserType>(true);
    } else {
      NDRegisterAccessor<UserType>::buffer_2D[0][0] =
          _fixedPointConverter.toCooked<UserType>(false);
    }
  ...
  void doPreWrite() override {
    ...
    if (!_fixedPointConverter.toRaw<UserType>(
            NDRegisterAccessor<UserType>::buffer_2D[0][0])) {
  ...
  bool mayReplaceOther(
    ...
    auto rhsCasted =
        boost::dynamic_pointer_cast<const LNMBackendBitAccessor<UserType>>(
            other);
    if (!rhsCasted)
      return false;
...
protected:
  ...
  void replaceTransferElement(
    ...
    auto casted =
        boost::dynamic_pointer_cast<NDRegisterAccessor<uint64_t>>(newElement);
