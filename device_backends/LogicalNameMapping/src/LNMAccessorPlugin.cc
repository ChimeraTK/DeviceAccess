...
namespace ChimeraTK { namespace LNMBackend {
  ...
  struct MathPluginDecorator : ChimeraTK::NDRegisterAccessorDecorator<UserType, double> {
    ...
    : ChimeraTK::NDRegisterAccessorDecorator<UserType, double>(target) {
      if(_target->getNumberOfChannels() != 1) {
        throw ChimeraTK::logic_error("The LogicalNameMapper MathPlugin supports only scalar or 1D array registers.");
      }
      exprtk::parser<double> parser;
      // add basic constants like pi
      symbols.add_constants();
      // add variable "x" for the register's value
      variables.push_back(buffer_2D[0]); // copy of the buffer, just to have the right size
      if(buffer_2D[0].size() == 1) {
        symbols.add_variable("x", variables.back());
      }
      else {
        symbols.add_vector("x", &(variables.back()), buffer_2D[0].size());
      }
      parser.compile(parameters.at("formula"), expression);
    }
  ...
  void MathPluginDecorator<UserType>::doPostRead() {
    _target->postRead();
    variables.back() = _target->accessChannel(0);
    for(size_t i = 0; i < _target->getNumberOfChannels(); ++i) {
      for(size_t k = 0; k < _target->getNumberOfSamples(); ++k) {
        buffer_2D[i][k] = doubleToUserType<UserType>(_target->accessData(i, k) * _factor);
      }
  ...
  struct MathPlugin_Helper {
    static boost::shared_ptr<NDRegisterAccessor<UserType>> decorateAccessor(
        boost::shared_ptr<NDRegisterAccessor<TargetType>>&, boost::shared_ptr<MathPluginImpl>&) {
      assert(false); // only specialisation is valid
      return {};
    }
  };
  ...
  struct MathPlugin_Helper<UserType, double> {
    static boost::shared_ptr<NDRegisterAccessor<UserType>> decorateAccessor(
        boost::shared_ptr<NDRegisterAccessor<double>>& target, boost::shared_ptr<MathPluginImpl>& impl) {
      return boost::make_shared<MultiplierPluginDecorator<UserType>>(target, impl);
    }
  };
  ...
  boost::shared_ptr<NDRegisterAccessor<UserType>> MathPlugin::decorateAccessor(
      boost::shared_ptr<NDRegisterAccessor<TargetType>>& target) const {
    return MultiplierPlugin_Helper<UserType, TargetType>::decorateAccessor(target, impl);
  }
