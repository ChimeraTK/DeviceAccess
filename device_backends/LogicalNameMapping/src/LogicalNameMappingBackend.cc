...
void LogicalNameMappingBackend::parse() {
  ...
  // parse the map fle
  LogicalNameMapParser parser =
      LogicalNameMapParser(_lmapFileName, _parameters);
...
LogicalNameMappingBackend::getRegisterAccessor_impl(
  ...
  auto info = boost::static_pointer_cast<LNMBackendRegisterInfo>(
      _catalogue_mutable.getRegister(registerPathName));
  ...
  if (info->targetType == LNMBackendRegisterInfo::TargetType::REGISTER) {
    ...
    ptr = boost::shared_ptr<NDRegisterAccessor<UserType>>(
        new LNMBackendChannelAccessor<UserType>(shared_from_this(),
                                                registerPathName, numberOfWords,
                                                wordOffsetInRegister, flags));
  } else if (info->targetType == LNMBackendRegisterInfo::TargetType::BIT) {
    ptr = boost::shared_ptr<NDRegisterAccessor<UserType>>(
        new LNMBackendBitAccessor<UserType>(shared_from_this(),
                                            registerPathName, numberOfWords,
                                            wordOffsetInRegister, flags));
  } else if (info->targetType ==
                 LNMBackendRegisterInfo::TargetType::INT_CONSTANT ||
             info->targetType ==
                 LNMBackendRegisterInfo::TargetType::INT_VARIABLE) {
    ptr = boost::shared_ptr<NDRegisterAccessor<UserType>>(
        new LNMBackendVariableAccessor<UserType>(
            shared_from_this(), registerPathName, numberOfWords,
            wordOffsetInRegister, flags));
  } else {
    throw ChimeraTK::logic_error(
        "For this register type, a RegisterAccessor cannot be obtained (name "
        "of logical register: " +
        registerPathName + ").");
  }
...
const RegisterCatalogue &
LogicalNameMappingBackend::getRegisterCatalogue() const {
  ...
  for (auto &info : _catalogue_mutable) {
    LNMBackendRegisterInfo &info_cast =
        static_cast<LNMBackendRegisterInfo &>(info);
    ...
    if (devName != "this") {
      target_info = _devices.at(devName)->getRegisterCatalogue().getRegister(
          std::string(info_cast.registerName));
    } else {
      target_info =
          _catalogue_mutable.getRegister(std::string(info_cast.registerName));
    }

    if (targetType != LNMBackendRegisterInfo::TargetType::BIT) {
      info_cast._dataDescriptor = target_info->getDataDescriptor();
    } else {
      info_cast._dataDescriptor = RegisterInfo::DataDescriptor(
          RegisterInfo::FundamentalType::boolean, true, false, 1, 0);
    }
    ...
    if ((int)info_cast.length == 0)
      info_cast.length = target_info->getNumberOfElements();
  }
