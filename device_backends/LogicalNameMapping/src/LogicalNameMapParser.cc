...
T LogicalNameMapParser::getValueFromXmlSubnode(const xmlpp::Node *node,
  ...
  if (list.size() < 1 && hasDefault)
    return defaultValue;
  if (list.size() != 1) {
    parsingError("Expected exactly one subnode of the type '" + subnodeName +
  ...
  for (auto &child : childList) {
    ...
    if (child->get_name() == "par") {
      auto childChildList = child->get_children();
      const xmlpp::TextNode *parNameNode =
          dynamic_cast<xmlpp::TextNode *>(childChildList.front());
...
std::vector<T> LogicalNameMapParser::getValueVectorFromXmlSubnode(
  ...
  auto list = node->find(subnodeName);
  if (list.size() < 1) {
    parsingError("Expected at least one subnode of the type '" + subnodeName +
                 "' below node '" + node->get_name() + "'.");
  }
  ...
  for (auto &child : list) {
    ...
    const xmlpp::TextNode *textNode =
        dynamic_cast<xmlpp::TextNode *>(childList.front());
    if (textNode) {
...
void LogicalNameMapParser::parseFile(const std::string &fileName) {
  ...
  try {
    ...
    throw ChimeraTK::logic_error("Error opening the xlmap file '" + fileName +
                                 "': " + e.what());
  }
  ...
  if (root->get_name() != "logicalNameMap") {
    parsingError("Expected 'logicalNameMap' tag instead of: " +
                 root->get_name());
  }
...
void LogicalNameMapParser::parseElement(RegisterPath currentPath,
  ...
  // register tag found: create new entry in map
    ...
    auto info =
        boost::shared_ptr<LNMBackendRegisterInfo>(new LNMBackendRegisterInfo());

    if (type == "redirectedRegister") {
      info->targetType = LNMBackendRegisterInfo::TargetType::REGISTER;
      info->deviceName =
          getValueFromXmlSubnode<std::string>(element, "targetDevice");
      info->registerName =
          getValueFromXmlSubnode<std::string>(element, "targetRegister");
      info->firstIndex = getValueFromXmlSubnode<unsigned int>(
          element, "targetStartIndex", true, 0);
      info->length = getValueFromXmlSubnode<unsigned int>(
          element, "numberOfElements", true, 0);
      info->nDimensions = 0;
      info->nChannels = 0;
      ...
      info->registerName =
          getValueFromXmlSubnode<std::string>(element, "targetRegister");
      info->channel =
          getValueFromXmlSubnode<unsigned int>(element, "targetChannel");
      info->firstIndex = 0;
      info->length = 0;
      info->nDimensions = 1;
      info->nChannels = 1;
      ...
      std::string constantType =
          getValueFromXmlSubnode<std::string>(element, "type");
      ...
      info->firstIndex = 0;
      info->length = getValueFromXmlSubnode<unsigned int>(
          element, "numberOfElements", true, 1);
      info->nDimensions = 0;
      info->nChannels = 1;
    ...
    RegisterPath registerName =
        currentPath / std::string(nameAttr->get_value());
...
std::unordered_set<std::string> LogicalNameMapParser::getTargetDevices() const {
  ...
  for (auto it = _catalogue.begin(); it != _catalogue.end(); ++it) {
    auto info =
        boost::static_pointer_cast<const LNMBackendRegisterInfo>(it.get());
