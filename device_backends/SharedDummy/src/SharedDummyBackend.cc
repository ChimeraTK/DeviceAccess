...
SharedDummyBackend::SharedDummyBackend(std::string instanceId,
...
void SharedDummyBackend::setupBarContents() {
  for (std::map<uint8_t, size_t>::const_iterator barSizeInBytesIter =
    ...
    std::string barName =
        SHARED_MEMORY_BAR_PREFIX + std::to_string(barSizeInBytesIter->first);

    size_t barSizeInWords =
        (barSizeInBytesIter->second + sizeof(int32_t) - 1) / sizeof(int32_t);

    try {
      std::lock_guard<boost::interprocess::named_mutex> lock(
          sharedMemoryManager.interprocessMutex);
      _barContents[barSizeInBytesIter->first] =
          sharedMemoryManager.findOrConstructVector(barName, barSizeInWords);
...
SharedDummyBackend::getBarSizesInBytesFromRegisterMapping() const {
...
void SharedDummyBackend::read(uint8_t bar, uint32_t address, int32_t *data,
                              size_t sizeInBytes) {
  if (!_opened) {
...
void SharedDummyBackend::write(uint8_t bar, uint32_t address,
...
std::string SharedDummyBackend::readDeviceInfo() {
  ...
  info << "SharedDummyBackend with mapping file "
...
boost::shared_ptr<DeviceBackend> SharedDummyBackend::createInstance(
  ...
  return returnInstance<SharedDummyBackend>(
      address, address, convertPathRelativeToDmapToAbs(mapFileName));
}

std::string SharedDummyBackend::convertPathRelativeToDmapToAbs(
  ...
  boost::filesystem::path absPathToMapFile{
      parserUtilities::concatenatePaths(absPathToDmapDir, mapfileName)};
