...
SharedDummyBackend::SharedMemoryManager::SharedMemoryManager(
    SharedDummyBackend &sharedDummyBackend_, const std::string &instanceId,
    ...
    : sharedDummyBackend(sharedDummyBackend_),
      ...
      name("ChimeraTK_SharedDummy_" + instanceIdHash + "_" + mapFileHash + "_" +
           userHash),
      segment(boost::interprocess::open_or_create, name.c_str(),
  ...
  auto pidSetData =
      segment.find<SharedMemoryVector>(SHARED_MEMORY_PID_SET_NAME);
  ...
  requiredVersion = segment.find_or_construct<unsigned>(
      SHARED_MEMORY_REQUIRED_VERSION_NAME)(0);
...
// Member functions
SharedMemoryVector *
SharedDummyBackend::SharedMemoryManager::findOrConstructVector(
    const std::string &objName, const size_t size) {
  SharedMemoryVector *vector = segment.find_or_construct<SharedMemoryVector>(
      objName.c_str())(size, 0, sharedMemoryIntAllocator);

  return vector;
}

size_t
SharedDummyBackend::SharedMemoryManager::getRequiredMemoryWithOverhead() {
  ...
  return SHARED_MEMORY_OVERHEAD_PER_VECTOR *
             sharedDummyBackend._barSizesInBytes.size() +
         SHARED_MEMORY_CONST_OVERHEAD +
         sharedDummyBackend.getTotalRegisterSizeInBytes();
}

std::pair<size_t, size_t>
SharedDummyBackend::SharedMemoryManager::getInfoOnMemory() {
...
SharedDummyBackend::SharedMemoryManager::listNamedElements() {
