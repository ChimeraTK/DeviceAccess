...
private:
  ...
  public:
    SharedMemoryManager(SharedDummyBackend &, const std::string &,
                        const std::string &);
    ~SharedMemoryManager();
     ...
     */
    SharedMemoryVector *findOrConstructVector(const std::string &objName,
                                              const size_t size);
  ...
  static std::map<std::string, boost::shared_ptr<DeviceBackend>> &
  getInstanceMap() {
