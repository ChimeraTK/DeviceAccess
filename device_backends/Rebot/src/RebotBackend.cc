...
void RebotBackend::write(uint8_t /*bar*/, uint32_t addressInBytes,
...
RebotBackend::createInstance(std::string /*address*/,
                             std::map<std::string, std::string> parameters) {
  if (parameters["ip"].size() == 0) {
    throw ChimeraTK::logic_error(
        "TMCB IP address not found in the parameter list");
  }
  if (parameters["port"].size() == 0) {
    throw ChimeraTK::logic_error(
        "TMCB port number not found in the parameter list");
  }
...
uint32_t
RebotBackend::parseRxServerHello(const std::vector<int32_t> &serverHello) {
  // 3 rd element/word is the version word
  return serverHello.at(2);
}

void RebotBackend::heartbeatLoop(
    boost::shared_ptr<ThreadInformerMutex> threadInformerMutex) {
  while (true) {
    try {
      ...
      // timeout (plus 1 ms)
      testable_rebot_sleep::sleep_until(
          _lastSendTime +
          boost::chrono::milliseconds(_connectionTimeout / 2 + 1));
    } catch (ChimeraTK::runtime_error &e) {
      std::cerr << "RebotBackend: Sending heartbeat failed. Caught exception: "
                << e.what() << std::endl;
