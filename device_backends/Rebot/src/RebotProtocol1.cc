...
RebotProtocol1::RebotProtocol1(boost::shared_ptr<TcpCtrl> &tcpCommunicator)
    : RebotProtocol0(tcpCommunicator),
...
void RebotProtocol1::read(uint32_t addressInBytes, int32_t *data,
                          size_t sizeInBytes) {
...
void RebotProtocol1::write(uint32_t addressInBytes, int32_t const *data,
                           size_t sizeInBytes) {
  RegisterInfo registerInfo(addressInBytes, sizeInBytes);
...
void RebotProtocol1::sendHeartbeat() {
  _tcpCommunicator->sendData(std::vector<uint32_t>(
      {HELLO_TOKEN, MAGIC_WORD, CLIENT_PROTOCOL_VERSION}));
