...
RebotProtocol0::RegisterInfo::RegisterInfo(uint32_t addressInBytes,
                                           uint32_t sizeInBytes) {
  if (sizeInBytes % 4 != 0) {
    throw ChimeraTK::logic_error(
        "\"size\" argument must be a multiplicity of 4");
  }
...
void RebotProtocol0::read(uint32_t addressInBytes, int32_t *data,
                          size_t sizeInBytes) {
  ...
  for (int count = 0; count < iterationsRequired; ++count) {
    fetchFromRebotServer(registerInfo.addressInWords +
                             (count * READ_BLOCK_SIZE),
                         READ_BLOCK_SIZE, data + (count * READ_BLOCK_SIZE));
  }
  // read remaining from the last multiple of READ_BLOCK_SIZE
  fetchFromRebotServer(
      registerInfo.addressInWords + (iterationsRequired * READ_BLOCK_SIZE),
      leftOverWords, data + (iterationsRequired * READ_BLOCK_SIZE));
}

void RebotProtocol0::write(uint32_t addressInBytes, int32_t const *data,
                           size_t sizeInBytes) {
  RegisterInfo registerInfo(addressInBytes, sizeInBytes);
  ...
  for (unsigned int i = 0; i < registerInfo.nWords; ++i) {
    ...
    // FIXME: Do proper host to network conversion, no manual, platform
    // dependent byte fiddling
...
void RebotProtocol0::fetchFromRebotServer(uint32_t wordAddress,
  ...
  if (responseCode[0] != rebot::READ_ACK) {
    ...
    throw ChimeraTK::runtime_error("Reading via ReboT failed. Response code: " +
                                   std::to_string(responseCode[0]));
  }
...
void RebotProtocol0::sendRebotReadRequest(const uint32_t wordAddress,
                                          const uint32_t wordsToRead) {
  unsigned int datasendSize = 3 * sizeof(int);
  std::vector<char> datasend(datasendSize);
  // FIXME: Do proper host to network conversion, no manual, platform dependent
  // byte fiddling send out an n word read request
...
void RebotProtocol0::transferVectorToDataPtr(std::vector<int32_t> source,
                                             int32_t *destination) {
