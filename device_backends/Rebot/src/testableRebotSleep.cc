...
namespace ChimeraTK { namespace testable_rebot_sleep {
  boost::chrono::steady_clock::time_point now() { return boost::chrono::steady_clock::now(); }
   ...
   * the test thread.
   */
  void sleep_until(boost::chrono::steady_clock::time_point t) { boost::this_thread::sleep_until(t); }
