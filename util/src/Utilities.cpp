...
bool Utilities::isSdm(std::string theString) {
  ...
  if (theString.length() < signatureLen)
    return false;
  if (theString.substr(0, 6) != "sdm://")
    return false;
  return true;
}
...
DeviceDescriptor Utilities::parseDeviceDesciptor(std::string cddString) {
  ...
  if (cddString.length() < 3) {
    throw ChimeraTK::logic_error(
        "Invalid ChimeraTK device descriptor (too short): " + cddString);
  }
  if (cddString.substr(0, 1) != "(") {
    throw ChimeraTK::logic_error(
        "Invalid ChimeraTK device descriptor (missing opening parenthesis): " +
        cddString);
  }
  ...
  for (auto &c : cddString) {
    ...
    if (parenthesesLevel == 1) {
      ...
      if (escapeNext) {
        if (c == ' ' || c == '?' || c == '&' || c == '(' || c == ')' ||
          ...
          throw ChimeraTK::logic_error(
              "Invalid ChimeraTK device descriptor (bad escape character): " +
              cddString);
        }
        escapeNext = false;
      ...
      if (c == '\\') {
        ...
        escapeNext = true;
      } else if (currentTokenType == tokenType::backendType &&
                 (c == ':' || c == '?' || c == ')')) {
      ...
      } else if (currentTokenType == tokenType::address &&
                 (c == '?' || c == ')')) {
        ...
        token = "";
        currentTokenType =
            tokenType::parameters; //  ')' will be processed below
      } else if (currentTokenType == tokenType::parameters &&
                 (c == '&' || c == ')')) {
        ...
        if (token.length() > 0) { // ignore empty parameter
          ...
          if (equalSign == std::string::npos) {
            throw ChimeraTK::logic_error(
                "Invalid ChimeraTK device descriptor (parameters must be "
          ...
          if (key.length() == 0) {
            throw ChimeraTK::logic_error(
                "Invalid ChimeraTK device descriptor (parameter key names must "
                "not be empty): " +
                cddString);
          }
          ...
          if (result.parameters.find(key) != result.parameters.end()) {
            throw ChimeraTK::logic_error(
                "Invalid ChimeraTK device descriptor (parameter '" + key +
    ...
    if (c == '(') {
      ...
      if (parenthesesLevel == 0 && positionPlusOne != cddString.length()) {
        ...
        throw ChimeraTK::logic_error(
            "Invalid ChimeraTK device descriptor (additional characters after "
  ...
  if (parenthesesLevel != 0) {
    throw ChimeraTK::logic_error(
        "Invalid ChimeraTK device descriptor (unmatched parenthesis): " +
        cddString);
  }
...
Sdm Utilities::parseDeviceString(std::string deviceString) {
  ...
  } else if ((boost::ends_with(deviceString, ".map")) ||
             (boost::ends_with(deviceString, ".mapp"))) {
...
DeviceInfoMap::DeviceInfo Utilities::aliasLookUp(std::string aliasName,
...
void Utilities::printStackTrace() {
  ...
  for (i = 0; i < trace_size; ++i) {
    ...
    if (status == 0) {
      ...
      std::cout << "[bt] #" << i << " (demangling failed) " << functionName
                << std::endl;
    }
  }
}
