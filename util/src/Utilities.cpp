    ...
    for(auto& c : cddString) {
        ...
        if(c == '\\') {
          ...
          if(token.length() == 0) {
            throw ChimeraTK::logic_error(
                "Invalid ChimeraTK device descriptor (backend type must be non-empty): " + cddString);
          }
          if(!boost::all(token, boost::is_alnum())) {
            throw ChimeraTK::logic_error(
                "Invalid ChimeraTK device descriptor (backend type must be alphanumeric): " + cddString);
          }
          ...
          if(token.length() > 0) { // ignore empty parameter
            ...
            if(equalSign == std::string::npos) {
              throw ChimeraTK::logic_error(
                  "Invalid ChimeraTK device descriptor (parameters must be specified as key=value pairs): " +
                  cddString);
            }
            ...
            if(key.length() == 0) {
              throw ChimeraTK::logic_error(
                  "Invalid ChimeraTK device descriptor (parameter key names must not be empty): " + cddString);
            }
            if(!boost::all(key, boost::is_alnum())) {
              throw ChimeraTK::logic_error("Invalid ChimeraTK device descriptor (parameter key names must contain only "
      ...
      if(c == '(') {
        ...
        if(parenthesesLevel == 0 && positionPlusOne != cddString.length()) {
          ...
          throw ChimeraTK::logic_error(
              "Invalid ChimeraTK device descriptor (additional characters after last closing parenthesis): " +
              cddString);
        }
      }
    }
