...
std::string Utilities::combinePaths(std::string &absoluteBasePath,
...
std::string Utilities::getAbsolutePathToDirectory(
    const std::string &fileName) {
  std::string absolutePathToDMapDir = getCurrentWorkingDirectory();
  size_t pos = fileName.find_last_of('/');
  if (pos != std::string::npos) {
    return combinePaths(absolutePathToDMapDir, fileName.substr(0, pos + 1)); // include the /
  }
  return absolutePathToDMapDir;
}

std::string Utilities::getAbsolutePathToFile(
    const std::string &fileName) {
  return getAbsolutePathToDirectory(fileName) + extractFileNameFromPath(fileName);
}

std::string Utilities::extractFileNameFromPath( const std::string &fileName) {
