...
template <template <typename> class TemplateClass> class TemplateUserTypeMap {
public:
  boost::fusion::map<
...
using SingleTypeUserTypeMap = boost::fusion::map<
    boost::fusion::pair<int8_t, T>, boost::fusion::pair<uint8_t, T>,
    boost::fusion::pair<int16_t, T>, boost::fusion::pair<uint16_t, T>,
    boost::fusion::pair<int32_t, T>, boost::fusion::pair<uint32_t, T>,
    boost::fusion::pair<int64_t, T>, boost::fusion::pair<uint64_t, T>,
    boost::fusion::pair<float, T>, boost::fusion::pair<double, T>,
...
#define DECLARE_MULTI_TEMPLATE_FOR_CHIMERATK_USER_TYPES(TemplateClass, ...)    \
  ...
  extern template class TemplateClass<                                         \
      std::string, __VA_ARGS__> // the last semicolon is added by the user
...
namespace detail {
template <typename X> class for_each_callable {
public:
  for_each_callable(X &fn) : fn_(fn) {}

  template <typename ARG_TYPE> void operator()(ARG_TYPE &argument) const {
    fn_(argument);
  }
...
void callForType(const std::type_info &type, LAMBDATYPE lambda) {
  ...
  if (!done) {
    ...
    throw myBadCast(
        std::string("ChimeraTK::callForType(): type is not known: ") +
        type.name());
  }
}
...
void callForType(const DataType &type, LAMBDATYPE lambda) {
  ...
  case DataType::none:
    ...
    throw myBadCast(std::string(
        "ChimeraTK::callForType() has been called for DataType::none"));
  }
}
