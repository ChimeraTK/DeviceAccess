...
#define DEFINE_VIRTUAL_FUNCTION_TEMPLATE_VTABLE(functionName, ...)             \
  template <typename T>                                                        \
  class functionName##_functionSignature                                       \
      : public boost::function<__VA_ARGS__> {                                  \
  public:                                                                      \
    boost::function<__VA_ARGS__> &                                             \
    operator=(const boost::function<__VA_ARGS__> &rhs) {                       \
...
#define CALL_VIRTUAL_FUNCTION_TEMPLATE(functionName, templateArgument, ...)    \
  boost::fusion::at_key<templateArgument>(functionName##_vtable.table)(        \
      __VA_ARGS__)
...
#define DEFINE_VIRTUAL_FUNCTION_TEMPLATE_VTABLE_FILLER(                        \
    className, functionName, numberOfArguments)                                \
  ...
  public:                                                                      \
    ...
    template <typename PAIR> void operator()(PAIR &pair) const {               \
      pair.second = boost::bind(                                               \
          &className::functionName<typename PAIR::first_type>, object,         \
...
#define FILL_VIRTUAL_FUNCTION_TEMPLATE_VTABLE(functionName)                    \
  boost::fusion::for_each(this->functionName##_vtable.table,                   \
                          functionName##_vtable_filler(this))
...
#define FILL_VIRTUAL_FUNCTION_TEMPLATE_VTABLE_STANDALONE(functionName,         \
  ...
  for_each(this->functionName##_vtable.table, [this](auto &pair) {             \
    typedef typename std::remove_reference<decltype(pair)>::type::first_type   \
        UserType;                                                              \
    pair.second =                                                              \
        boost::bind(&ClassName::functionName<UserType>, this,                  \
                    DEFINE_TEMPLATE_VTABLE_FILLER_HELPER_##numberOfArguments); \
  });
