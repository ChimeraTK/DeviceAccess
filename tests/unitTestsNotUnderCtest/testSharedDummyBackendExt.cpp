...
struct TestFixture {
  TestFixture()
      : argc(framework::master_test_suite().argc),
...
BOOST_FIXTURE_TEST_CASE(testRobustnessMain, TestFixture) {
  ...
  const unsigned maxIncorrectIterations =
      20; /* Timeout while waiting for 2nd application */
  ...
  do {
    ...
    } while ((std::vector<int>)processVarsRead ==
                 (std::vector<int>)processVarsOld &&
             !waitingForResponse);

    if ((std::vector<int>)processVarsWrite ==
        (std::vector<int>)processVarsRead) {
    ...
    processVarsOld = processVarsWrite;
           ...
           incorrectIterations != maxIncorrectIterations &&
           iterations != nIterations);

  BOOST_CHECK(readbackCorrect);
  std::cout << "Finished test after " << iterations << " of " << nIterations
            << " Iterations." << std::endl;
  if (incorrectIterations == maxIncorrectIterations) {
    std::cout << "Test cancelled because echoing process did not respond!"
              << std::endl;
  }
...
BOOST_FIXTURE_TEST_CASE(testReadWrite, TestFixture) {
  ...
  boost::filesystem::path absPathToMapFile =
      boost::filesystem::absolute(mapFileName);

  std::string shmName{
      createExpectedShmName(instanceId, absPathToMapFile.string())};
...
BOOST_AUTO_TEST_CASE(testMakeMess) {
  ...
  int n = 0;
  std::generate(processVars.begin(), processVars.end(),
                [n]() mutable { return n++; });
  processVars.write();
...
BOOST_AUTO_TEST_CASE(testVerifyCleanup) {
  ...
  BOOST_CHECK(std::all_of(processVars.begin(), processVars.end(),
                          [](int i) { return i == 0; }));
...
static void interrupt_handler(int signal) {
  std::cout << "Caught interrupt signal (" << signal << "). Terminating..."
            << std::endl;
  terminationCaught = true;
}

// Static helper functions
static std::string createExpectedShmName(std::string instanceId_,
  ...
  return "ChimeraTK_SharedDummy_" + instanceIdHash + "_" + mapFileHash + "_" +
         userHash;
}
