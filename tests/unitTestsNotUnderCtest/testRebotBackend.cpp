...
public:
  explicit RebotDeviceTestSuite(std::string const &cardAlias)
    ...
    add(BOOST_CLASS_TEST_CASE(&RebotTestClass::testReadWriteAPIOfRebotBackend,
                              rebotTest));
  }
};
...
RebotServerDetails
RebotTestClass::getServerDetails(const std::string &cardAlias) {
  DeviceInfo deviceDetails = getDeviceDetailsFromDMap(cardAlias);
  return extractServerDetailsFromUri(deviceDetails.uri);
}

DeviceInfo
RebotTestClass::getDeviceDetailsFromDMap(const std::string &cardAlias) {
...
RebotServerDetails
RebotTestClass::extractServerDetailsFromUri(std::string &uri) {
...
void RebotTestClass::testConnection() { // BAckend test
  ...
  ChimeraTK::RebotBackend secondConnectionToServer(_rebotServer.ip,
                                                   _rebotServer.port);
...
void RebotTestClass::testReadWriteAPIOfRebotBackend() {
  ...
  rebotBackend.read(0, word_status_register_address, &readValue,
                    sizeof(readValue));
  ...
  rebotBackend.write(0, test_area_Addr, test_area_data.data(),
                     sizeof(int32_t) * test_area_data.size());
  rebotBackend.read(0, test_area_Addr, test_area_ReadIndata.data(),
