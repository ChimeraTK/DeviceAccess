...
public:
  OneDRegisterTestSuite() : test_suite("OneDRegisterAccessor test suite") {
    ...
    boost::shared_ptr<OneDRegisterTest> bufferingRegisterTest(
        new OneDRegisterTest);

    add(BOOST_CLASS_TEST_CASE(&OneDRegisterTest::testRegisterAccessor,
...
void OneDRegisterTest::testRegisterAccessor() {
  ...
  OneDRegisterAccessor<int> intRegister =
      device->getOneDRegisterAccessor<int>("APP0/MODULE0");
  ...
  intRegister[2] = 222;
  intRegister.write();
  BOOST_CHECK(device->read<int>("APP0/MODULE0", 3) ==
              std::vector<int>({-666, 999, 222}));
  ...
  int ic = 0;
  for (OneDRegisterAccessor<int>::iterator it = intRegister.begin();
  ...
  ic = 0;
  for (OneDRegisterAccessor<int>::reverse_iterator it = intRegister.rbegin();
  ...
  ic = 0;
  for (OneDRegisterAccessor<int>::const_iterator it = const_intRegister.begin();
       it != const_intRegister.end(); ++it) {
    BOOST_CHECK(*it == 1234 * (ic + 1));
    ic++;
  }
  ic = 0;
  for (OneDRegisterAccessor<int>::const_reverse_iterator it =
  ...
  OneDRegisterAccessor<double> floatRegister =
      device->getOneDRegisterAccessor<double>("MODULE0/WORD_USER1");
  ...
  floatRegister.write();
  BOOST_CHECK(device->read<int>("APP0/MODULE0", 2) ==
              std::vector<int>({42, 2468}));
}
