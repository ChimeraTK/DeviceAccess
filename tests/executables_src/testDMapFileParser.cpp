...
void DMapFileParserTest::testParseFile() {
    ...
    std::string absPathToDmap = mtca4u::Utilities::getAbsolutePathToFile("valid.dmap");
    std::string absPathToDmapDir = mtca4u::Utilities::getAbsolutePathToDirectory("valid.dmap");

	populateDummyDeviceInfo(deviceInfo1, absPathToDmap, "card1", "/dev/dev1",
	    mtca4u::Utilities::combinePaths(absPathToDmapDir, "goodMapFile_withoutModules.map"));
