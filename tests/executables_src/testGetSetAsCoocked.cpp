...
BOOST_AUTO_TEST_CASE( testRawAccessor ){
  ...
  oneDRawAccessor[1]=13;
   ...
   // the register has 3 fractional bits
  BOOST_CHECK( std::fabs(oneDRawAccessor.getAsCoocked<double>(0) - 12./8) < 0.0001);
  BOOST_CHECK( std::fabs(oneDRawAccessor.getAsCoocked<double>(1) - 13./8) < 0.0001);

  oneDRawAccessor.setAsCoocked(0,42./8);
  oneDRawAccessor.setAsCoocked(1,43./8);
