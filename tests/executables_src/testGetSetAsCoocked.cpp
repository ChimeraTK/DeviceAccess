...
BOOST_AUTO_TEST_CASE(testRawAccessor) {
  ...
  auto scalarRawAccessor = d.getScalarRegisterAccessor<int32_t>(
      "BOARD/WORD_USER", 0, {AccessMode::raw});
  scalarRawAccessor = 25;
  // the register has 3 fractional bits
  BOOST_CHECK(std::fabs(scalarRawAccessor.getAsCooked<double>() - 25. / 8) <
              0.0001);

  scalarRawAccessor.setAsCooked(31. / 8);
  BOOST_CHECK_EQUAL(int32_t(scalarRawAccessor), 31);

  auto oneDRawAccessor = d.getOneDRegisterAccessor<int32_t>(
  ...
  // the register has 3 fractional bits
  BOOST_CHECK(std::fabs(oneDRawAccessor.getAsCooked<double>(0) - 12. / 8) <
              0.0001);
  BOOST_CHECK(std::fabs(oneDRawAccessor.getAsCooked<double>(1) - 13. / 8) <
              0.0001);
