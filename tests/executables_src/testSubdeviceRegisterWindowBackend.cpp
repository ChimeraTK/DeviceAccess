...
BOOST_AUTO_TEST_CASE(TestRegWindowRead) {
  setDMapFilePath("registerWindow.dmap");

  ChimeraTK::Device device("REG_WINDOW_SUBDEV");
  device.open();

  auto acc1 = device.getScalarRegisterAccessor<int16_t>("SHORT", 0, {AccessMode::raw});
  acc1.read();
  // std::cout << std::hex << acc1 << std::dec << std::endl;
}
