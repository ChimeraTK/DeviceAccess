...
public:
  registerCatalogueTestSuite()
      : test_suite("RegisterCatalogue class test suite") {
    boost::shared_ptr<RegisterCatalogueTest> test(new RegisterCatalogueTest());

    add(BOOST_CLASS_TEST_CASE(&RegisterCatalogueTest::testRegisterCatalogue,
                              test));
  }
};
...
bool init_unit_test() {
  framework::master_test_suite().p_name.value =
      "RegisterCatalogue class test suite";
...
public:
  myRegisterInfo(std::string path, unsigned int nbOfElements,
                 unsigned int nbOfChannels, unsigned int nbOfDimensions,
                 DataDescriptor dataDescriptor, bool readable, bool writeable,
                 AccessModeFlags supportedFlags)
      : _path(path), _nbOfElements(nbOfElements), _nbOfChannels(nbOfChannels),
        _nbOfDimensions(nbOfDimensions), _dataDescriptor(dataDescriptor),
...
void RegisterCatalogueTest::testRegisterCatalogue() {
  ...
  RegisterInfo::DataDescriptor dataDescriptor(
      RegisterInfo::FundamentalType::numeric, false, false, 8, 3,
      DataType::int32);
  catalogue.addRegister(boost::shared_ptr<RegisterInfo>(
      new myRegisterInfo("/some/register/name", 42, 3, 2, dataDescriptor, true,
  ...
  BOOST_CHECK(info->getDataDescriptor().fundamentalType() ==
              RegisterInfo::FundamentalType::numeric);
  ...
  BOOST_CHECK(info->getSupportedAccessModes().has(
                  AccessMode::wait_for_new_data) == false);

  RegisterInfo::DataDescriptor dataDescriptor2(
      RegisterInfo::FundamentalType::numeric, true, false, 12);
  catalogue.addRegister(boost::shared_ptr<RegisterInfo>(new myRegisterInfo(
      "/some/other/name", 1, 1, 0, dataDescriptor2, true, true,
      {AccessMode::raw, AccessMode::wait_for_new_data})));
  ...
  BOOST_CHECK(info->getSupportedAccessModes().has(AccessMode::raw) == true);
  BOOST_CHECK(info->getSupportedAccessModes().has(
                  AccessMode::wait_for_new_data) == true);

  RegisterInfo::DataDescriptor dataDescriptor3(
      RegisterInfo::FundamentalType::string);
  catalogue.addRegister(boost::shared_ptr<RegisterInfo>(new myRegisterInfo(
      "/justAName", 1, 1, 0, dataDescriptor3, false, false, {})));
