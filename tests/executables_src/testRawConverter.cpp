...
// Adapter class so we can use the FixedPointConverter tests unmodified for the new RawConverter
template<typename RawType>
class FixedPointConverter {
 public:
  FixedPointConverter(
      std::string variableName, unsigned int nBits = 32, int fractionalBits = 0, bool isSignedFlag = true)
  : info(variableName, 1, 0, 4, 0, nBits, fractionalBits, isSignedFlag) {}
  ...
  UserType scalarToCooked(RawType raw) const {
    UserType rv;
    RawConverter::withConverter<UserType, RawTypeUnsigned>(
        info, 0, [&](auto converter) { rv = converter.toCooked(RawTypeUnsigned(raw)); });
    return rv;
  }
  ...
  RawType toRaw(UserType cooked) const {
    RawTypeUnsigned rv;
    RawConverter::withConverter<UserType, RawTypeUnsigned>(
        info, 0, [&](auto converter) { rv = converter.toRaw(cooked); });
    return RawType(rv);
  }

 private:
  ChimeraTK::NumericAddressedRegisterInfo info;
};
...
BOOST_AUTO_TEST_CASE(testInt32_fractionMinus12) {
  ...
  checkToRaw(converter, SIGNED_HEX_TO_DOUBLE(0xAAAAAAAA), 0xFFFAAAAB);
...
/**********************************************************************************************************************/
/*      FIXED POINT TEST SECTION                                                                                      */
/**********************************************************************************************************************/

// Adapter class so we can use the IEEE754_SingleConverter tests unmodified for the new RawConverter
class IEEE754_SingleConverter {
 public:
  IEEE754_SingleConverter()
  : _info("", 1, 0, 4, 0, 32, 0, true, ChimeraTK::NumericAddressedRegisterInfo::Access::READ_WRITE,
        ChimeraTK::NumericAddressedRegisterInfo::Type::IEEE754) {}

  template<typename UserType>
  [[nodiscard]] UserType scalarToCooked(int32_t raw) const {
    UserType rv;
    RawConverter::withConverter<UserType, uint32_t>(
        _info, 0, [&](auto converter) { rv = converter.toCooked(uint32_t(raw)); });
    return rv;
  }

  template<typename UserType>
  [[nodiscard]] int32_t toRaw(UserType cooked) const {
    uint32_t rv;
    RawConverter::withConverter<UserType, uint32_t>(_info, 0, [&](auto converter) { rv = converter.toRaw(cooked); });
    return int32_t(rv);
  }

 private:
  ChimeraTK::NumericAddressedRegisterInfo _info;
};
