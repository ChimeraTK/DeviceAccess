...
class DMapFilesParserTestSuite : public test_suite {
 public:
  DMapFilesParserTestSuite() : test_suite("dMapFilesParser class test suite") {
    boost::shared_ptr<DMapFilesParserTest> DMapFilesParserTestPtr(new DMapFilesParserTest);

    // test parsing of dmap file in the current directory
    add(BOOST_TEST_CASE(boost::bind(&DMapFilesParserTest::testParseFile, DMapFilesParserTestPtr, "")));
    add(BOOST_TEST_CASE(boost::bind(&DMapFilesParserTest::testParseFile, DMapFilesParserTestPtr, "dMapDir/")));
    add(BOOST_TEST_CASE(boost::bind(&DMapFilesParserTest::testParseFile, DMapFilesParserTestPtr,
        ChimeraTK::parserUtilities::getCurrentWorkingDirectory() + "/")));
    // test with an empty, existing file
    add(BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testParseEmptyDmapFile, DMapFilesParserTestPtr));
    // test with a non-existing file
    add(BOOST_TEST_CASE(
        boost::bind(&DMapFilesParserTest::testParseNonExistentDmapFile, DMapFilesParserTestPtr, "notExisting.dmap")));
    // special case: file in the root directory. Cannot be there in the test, an
    // probably also not in real life.
    add(BOOST_TEST_CASE(
        boost::bind(&DMapFilesParserTest::testParseNonExistentDmapFile, DMapFilesParserTestPtr, "/some.dmap")));

    test_case* testGetMapFile = BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testGetMapFile, DMapFilesParserTestPtr);
    test_case* testRegisterInfo =
        BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testGetRegisterInfo, DMapFilesParserTestPtr);
    test_case* testGetDMapFileSize =
        BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testGetDMapFileSize, DMapFilesParserTestPtr);
    test_case* testCheckParsedInInfo =
        BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testCheckParsedInInfo, DMapFilesParserTestPtr);
    test_case* testOverloadedStreamOperator =
        BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testOverloadedStreamOperator, DMapFilesParserTestPtr);
    test_case* testIteratorBeginEnd =
        BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testIteratorBeginEnd, DMapFilesParserTestPtr);

    test_case* testParsedirInvalidDir =
        BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testParsedirInvalidDir, DMapFilesParserTestPtr);
    test_case* testParseEmptyDirectory =
        BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testParseEmptyDirectory, DMapFilesParserTestPtr);
    test_case* testParseDirectoryWithBlankDMap =
        BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testParseDirectoryWithBlankDMap, DMapFilesParserTestPtr);
    test_case* testParseDirWithGoodDmaps =
        BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testParseDirWithGoodDmaps, DMapFilesParserTestPtr);
    test_case* testParseDirs = BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testParseDirs, DMapFilesParserTestPtr);

    test_case* testConstructor = BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testConstructor, DMapFilesParserTestPtr);
    test_case* testMapException = BOOST_CLASS_TEST_CASE(&DMapFilesParserTest::testMapException, DMapFilesParserTestPtr);
...
bool init_unit_test() {
  framework::master_test_suite().p_name.value = "dMapFilesParser class test suite";
...
void DMapFilesParserTest::testParseFile(std::string pathToDmapFile) {
  ...
  std::string absolutePathToDMapDir = ChimeraTK::parserUtilities::concatenatePaths(currentWorkingDir, pathToDmapFile);

  populateDummyDeviceInfo(expectedDeviceInfo1, absolutePathToDMapFile, "card1", "/dev/dev1",
      ChimeraTK::parserUtilities::concatenatePaths(absolutePathToDMapDir, path_to_map_file1));
  populateDummyDeviceInfo(expectedDeviceInfo2, absolutePathToDMapFile, "card2", "/dev/dev2",
      ChimeraTK::parserUtilities::concatenatePaths(absolutePathToDMapDir, path_to_map_file2));
  populateDummyDeviceInfo(expectedDeviceInfo3, absolutePathToDMapFile, "card3", "/dev/dev3",
      ChimeraTK::parserUtilities::concatenatePaths(absolutePathToDMapDir, path_to_map_file3));
  populateDummyDeviceInfo(
      expectedDeviceInfo4, absolutePathToDMapFile, "card4", "(pci:mtcadummys0?map=goodMapFile_withoutModules.map)", "");
  ...
  BOOST_CHECK(compareDeviceInfos(expectedDeviceInfo1, reterievedDeviceInfo1) == true);

  filesParser.getdMapFileElem(1, reterievedDeviceInfo2);
  BOOST_CHECK(compareDeviceInfos(expectedDeviceInfo2, reterievedDeviceInfo2) == true);

  filesParser.getdMapFileElem(2, reterievedDeviceInfo3);
  BOOST_CHECK(compareDeviceInfos(expectedDeviceInfo3, reterievedDeviceInfo3) == true);

  filesParser.getdMapFileElem(3, reterievedDeviceInfo4);
  BOOST_CHECK(compareDeviceInfos(expectedDeviceInfo4, reterievedDeviceInfo4) == true);

  BOOST_CHECK_THROW(filesParser.getdMapFileElem(4, reterievedDeviceInfo5), ChimeraTK::logic_error);

  ChimeraTK::DeviceInfoMap::DeviceInfo reterievedDeviceInfo6 = filesParser.getdMapFileElem("card2");

  BOOST_CHECK(compareDeviceInfos(expectedDeviceInfo2, reterievedDeviceInfo6) == true);

  BOOST_CHECK_THROW(filesParser.getdMapFileElem("card_not_present"), ChimeraTK::logic_error);

  ChimeraTK::DeviceInfoMap::DeviceInfo reterievedDeviceInfo7;
  filesParser.getdMapFileElem("card2", reterievedDeviceInfo7);
  BOOST_CHECK(compareDeviceInfos(expectedDeviceInfo2, reterievedDeviceInfo7) == true);
}
...
void DMapFilesParserTest::testParseEmptyDmapFile() {
  ...
  BOOST_CHECK_THROW(filesParser.parse_file(dmapFileName), ChimeraTK::logic_error);
}
...
void DMapFilesParserTest::testGetMapFile() {
  ...
  for(it = map_file_for_card1->begin(), index = 0; it != map_file_for_card1->end(); ++it, ++index) {
  ...
  for(it = map_file_for_card3->begin(), index = 0; it != map_file_for_card3->end(); ++it, ++index) {
    BOOST_CHECK((compareRegisterInfoents(*ptrList[index], *it)) == true);
  }

  BOOST_CHECK_THROW(filesParser.getMapFile("card_unknown"), ChimeraTK::logic_error);
}
...
void DMapFilesParserTest::testGetRegisterInfo() {
  ...
  ChimeraTK::RegisterInfoMap::RegisterInfo RegisterInfoent3(
      "WORD_STATUS", 0x00000001, 0x00000008, 0x00000004, 0x00000000, 32, 0, true);
  filesParser.getRegisterInfo("card1", "WORD_STATUS", retrivedDeviceFileName, retrivedRegisterInfo);
  ...
  filesParser.getRegisterInfo("card3", "WORD_STATUS", retrivedDeviceFileName, retrivedRegisterInfo);

  BOOST_CHECK(retrivedDeviceFileName == "/dev/dev3");
  BOOST_CHECK(compareRegisterInfoents(retrivedRegisterInfo, RegisterInfoent3) == true);

  BOOST_CHECK_THROW(
      filesParser.getRegisterInfo("card_unknown", "WORD_STATUS", retrivedDeviceFileName, retrivedRegisterInfo),
  ...
  filesParser2.getRegisterInfo("", "WORD_STATUS", retrivedDeviceFileName, retrivedRegisterInfo);
  BOOST_CHECK(retrivedDeviceFileName == "/dev/dev1");
  BOOST_CHECK(compareRegisterInfoents(retrivedRegisterInfo, RegisterInfoent3) == true);
  ...
  filesParser.getRegisterInfo(
      "card2", "WORD_STATUS", retrivedDeviceFileName, retrivedElemNr, retrivedOffset, retrivedRegSize, retrivedRegBar);
  ...
  filesParser2.getRegisterInfo(
      "", "WORD_STATUS", retrivedDeviceFileName, retrivedElemNr, retrivedOffset, retrivedRegSize, retrivedRegBar);
  BOOST_CHECK(retrivedDeviceFileName == "/dev/dev1");
  BOOST_CHECK(retrivedElemNr == 1);
  BOOST_CHECK(retrivedOffset == 0x00000008);
  BOOST_CHECK(retrivedRegSize == 0x00000004);
  ...
  BOOST_CHECK_THROW(filesParser.getRegisterInfo("unknown_card", "WORD_STATUS", retrivedDeviceFileName, retrivedElemNr,
...
void DMapFilesParserTest::testCheckParsedInInfo() {
  ...
  bool returnValue = filesParser1.check(ChimeraTK::DeviceInfoMap::ErrorList::ErrorElem::ERROR,
      ChimeraTK::RegisterInfoMap::ErrorList::ErrorElem::WARNING, dmap_err_list, map_err_list);
  BOOST_CHECK(returnValue == true);

  bool status = filesParser.check(ChimeraTK::DeviceInfoMap::ErrorList::ErrorElem::ERROR,
      ChimeraTK::RegisterInfoMap::ErrorList::ErrorElem::WARNING, dmap_err_list, map_err_list);
  ...
  std::list<ChimeraTK::DeviceInfoMap::ErrorList::ErrorElem>::iterator errorIterator = dmap_err_list._errors.begin();
  BOOST_CHECK((errorIterator->_errorDevice1.deviceName == errorIterator->_errorDevice2.deviceName));

  BOOST_CHECK(map_err_list.errors.size() == 2);
  std::list<ChimeraTK::RegisterInfoMap::ErrorList::ErrorElem>::iterator mapErrIt;
  mapErrIt = map_err_list.errors.begin();

  bool areNonUniqueRegistersPresent = ((mapErrIt->_errorRegister1.name == mapErrIt->_errorRegister2.name) &&
...
void DMapFilesParserTest::testOverloadedStreamOperator() {
  ChimeraTK::DMapFilesParser filesParser;
  std::string path_to_dmap_file = "dMapDir/valid.dmap";
  std::string absPathToDMapFile = ChimeraTK::parserUtilities::convertToAbsolutePath(path_to_dmap_file);
  std::string absPathToDMapDir = ChimeraTK::parserUtilities::getCurrentWorkingDirectory() + "dMapDir";
  filesParser.parse_file(path_to_dmap_file);
  ...
  populateDummyDeviceInfo(deviceInfo1, absPathToDMapFile, "card1", "/dev/dev1",
      ChimeraTK::parserUtilities::concatenatePaths(absPathToDMapDir, "goodMapFile_withoutModules.map"));
  populateDummyDeviceInfo(deviceInfo2, absPathToDMapFile, "card2", "/dev/dev2",
      ChimeraTK::parserUtilities::concatenatePaths(absPathToDMapDir, "./goodMapFile_withoutModules.map"));
  populateDummyDeviceInfo(deviceInfo3, absPathToDMapFile, "card3", "/dev/dev3",
      ChimeraTK::parserUtilities::getCurrentWorkingDirectory() + "goodMapFile_withoutModules.map");
  populateDummyDeviceInfo(
      deviceInfo4, absPathToDMapFile, "card4", "(pci:mtcadummys0?map=goodMapFile_withoutModules.map)", "");
...
void DMapFilesParserTest::testIteratorBeginEnd() {
  ...
  std::string currentWrkingDir = ChimeraTK::parserUtilities::getCurrentWorkingDirectory();
  ...
  std::vector<std::pair<ChimeraTK::DeviceInfoMap::DeviceInfo, ChimeraTK::RegisterInfoMapPointer>>::iterator iter;
  std::vector<std::pair<ChimeraTK::DeviceInfoMap::DeviceInfo, ChimeraTK::RegisterInfoMapPointer>>::const_iterator
      const_iter;
  uint8_t i;
  for(iter = filesParser.begin(), i = 0; (iter != filesParser.end()) && (i < 3); ++i, ++iter) {
    BOOST_CHECK(compareDeviceInfos(*tmpArray1[i], (*iter).first) == true);
    BOOST_CHECK(*tmpArray2[i] == (*iter).second->getMapFileName());
  }
  for(const_iter = constfilesParser.begin(), i = 0; (const_iter != constfilesParser.end()) && (i < 3);
      ++i, ++const_iter) {
...
void DMapFilesParserTest::testParseDirWithGoodDmaps() {
  ...
  std::string absPathToDmap1 = ChimeraTK::parserUtilities::convertToAbsolutePath("./GoodDmapDir/first.dmap");
  std::string absPathToDmap2 = ChimeraTK::parserUtilities::convertToAbsolutePath("./GoodDmapDir/second.dmap");
  std::string absPathToDmap3 = ChimeraTK::parserUtilities::convertToAbsolutePath("./GoodDmapDir/second.dmap");
  std::string absPathToDmap4 = ChimeraTK::parserUtilities::convertToAbsolutePath("./GoodDmapDir/first.dmap");

  std::string absPathToDmapDir = ChimeraTK::parserUtilities::getCurrentWorkingDirectory() + "./GoodDmapDir";

  populateDummyDeviceInfo(expectedDeviceInfo1, absPathToDmap1, "card1", "/dev/dev1",
      ChimeraTK::parserUtilities::concatenatePaths(absPathToDmapDir, "./mapFile1.map"));
  populateDummyDeviceInfo(expectedDeviceInfo2, absPathToDmap2, "card2", "/dev/dev2",
      ChimeraTK::parserUtilities::concatenatePaths(absPathToDmapDir, "./mapFile2.map"));

  populateDummyDeviceInfo(expectedDeviceInfo3, absPathToDmap3, "card3", "/dev/dev3",
      ChimeraTK::parserUtilities::concatenatePaths(absPathToDmapDir, "./mapFile2.map"));
  populateDummyDeviceInfo(expectedDeviceInfo4, absPathToDmap4, "card4", "/dev/dev4",
      ChimeraTK::parserUtilities::concatenatePaths(absPathToDmapDir, "mtcadummy_withoutModules.map"));
...
inline void DMapFilesParserTest::testConstructor() {
  ...
  populateDummyDeviceInfo(expectedDeviceInfo1, absPathToDmap1, "card1", "/dev/dev1",
      ChimeraTK::parserUtilities::concatenatePaths(absPathToDmapDir, "./mapFile1.map"));
  populateDummyDeviceInfo(expectedDeviceInfo3, absPathToDmap2, "card3", "/dev/dev3",
      ChimeraTK::parserUtilities::concatenatePaths(absPathToDmapDir, "./mapFile2.map"));
...
void DMapFilesParserTest::testMapException() {
  BOOST_CHECK_THROW(ChimeraTK::DMapFilesParser filesParser("./emptyMapFile"), ChimeraTK::logic_error);
}
