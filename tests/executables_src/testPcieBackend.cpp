...
std::string PcieBackendTest::checkDmaValues(std::vector<int32_t> const& dmaBuffer) {
  ...
  errorMessage << "Content of transferred DMA block is not valid. First wrong value at index " << i << " is "
               << dmaBuffer[i] << std::endl;
...
void PcieBackendTest::testReadDMA() {
  ...
  // Start the ADC on the dummy device. This will fill the "DMA" buffer with the default values (index^2) in the first
  // 25 words.
  ...
  _pcieBackendInstance->read(/*the dma bar*/ 2, /*offset*/ readOffset * sizeof(int32_t), &smallBuffer[0],
...
void PcieBackendTest::testRead() {
  ...
  // Start the ADC on the dummy device. This will fill bar 2 (the "DMA" buffer) with the default values (index^2) in the
  // first 25 words.
  int32_t data = 1;
  _pcieBackendInstance->write(/*bar*/ 0, WORD_ADC_ENA_OFFSET, &data, 4);
