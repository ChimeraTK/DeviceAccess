...
struct TestLocker {
  TestLocker() {
    ...
    if (fd == -1) {
      std::cout << "Cannot open file 'shareddummyTest.dmap' for locking."
                << std::endl;
      exit(1);
    }
...
BOOST_AUTO_TEST_CASE(testReadWrite) {
  ...
  int n = 0;
  std::generate(processVars11.begin(), processVars11.end(),
                [n]() mutable { return n++; });
  processVars11.write();
  processVars11.read();
  ...
  n = 0;
  std::generate(processVars23.begin(), processVars23.end(),
                [n]() mutable { return n++; });
  processVars23.write();
  processVars23.read();
  ...
  n = 0;
  std::generate(processVarsWrite21.begin(), processVarsWrite21.end(),
                [n]() mutable { return n++; });
  processVarsWrite21.write();

  // start second accessing application
  BOOST_CHECK(
      !std::system("./testSharedDummyBackendExt "
  ...
  BOOST_CHECK((std::vector<int>)processVarsWrite21 ==
