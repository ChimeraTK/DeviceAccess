...
BOOST_AUTO_TEST_CASE(testCreateBackend) {
  ...
  BOOST_CHECK_THROW(BackendFactory::getInstance().createBackend("test"), ChimeraTK::logic_error); // dmap file not found
                                                                                                  // exception .
  BackendFactory::getInstance().setDMapFilePath(oldtestFilePath);
  BOOST_CHECK_THROW(BackendFactory::getInstance().createBackend("test"), ChimeraTK::logic_error); // file found but not
                                                                                                  // an existing alias.
  ...
  BOOST_CHECK_THROW(BackendFactory::getInstance().createBackend("test"), ChimeraTK::logic_error); // not an existing
                                                                                                  // alias.
