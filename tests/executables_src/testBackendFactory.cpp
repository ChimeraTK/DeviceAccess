...
struct NewBackend : public DummyBackend {
  ...
  createInstance(std::string /*host*/, std::string instance,
    ...
    return returnInstance<NewBackend>(
...
BOOST_AUTO_TEST_CASE(testCreateBackend) {
  ...
  std::string invalidtestFilePath =
      std::string(TEST_DMAP_FILE_PATH) + "disabled";
  BOOST_CHECK_THROW(
  ...
  BOOST_CHECK_NO_THROW(testPtr = BackendFactory::getInstance().createBackend(
                           "DUMMYD0")); // entry in old dummies.dmap
  BOOST_CHECK(testPtr);
  testPtr.reset();
  ...
  BOOST_CHECK_NO_THROW(testPtr2 = BackendFactory::getInstance().createBackend(
                           "DUMMYD9")); // open existing backend
                                        // again
...
BOOST_AUTO_TEST_CASE(testPluginMechanism) {
  ...
  BOOST_CHECK_THROW(BackendFactory::getInstance().createBackend(
                        "sdm://./newBackend=goodMapFile.map"),
                    ChimeraTK::logic_error);

  BOOST_CHECK_NO_THROW(
      ChimeraTK::BackendFactory::getInstance().registerBackendType(
          "newBackend", "", &NewBackend::createInstance,
          CHIMERATK_DEVICEACCESS_VERSION));

  BOOST_CHECK_NO_THROW(BackendFactory::getInstance().createBackend(
      "sdm://./newBackend=goodMapFile.map"));

  BOOST_CHECK_THROW(ChimeraTK::BackendFactory::getInstance().loadPluginLibrary(
                        "notExisting.so"),
                    ChimeraTK::logic_error);

  BOOST_CHECK_NO_THROW(
      ChimeraTK::BackendFactory::getInstance().loadPluginLibrary(
          "./libWorkingBackend.so"));
  ...
  BOOST_CHECK_THROW(ChimeraTK::BackendFactory::getInstance().loadPluginLibrary(
                        "libNotRegisteringPlugin.so"),
                    ChimeraTK::logic_error);
  BOOST_CHECK_THROW(BackendFactory::getInstance().createBackend(
                        "sdm://./notRegisteringPlugin=goodMapFile.map"),
                    ChimeraTK::logic_error);

  BOOST_CHECK_NO_THROW(
      ChimeraTK::BackendFactory::getInstance().loadPluginLibrary(
          "./libWrongVersionBackend.so"));
  BOOST_CHECK_THROW(BackendFactory::getInstance().createBackend(
                        "sdm://./wrongVersionBackend=goodMapFile.map"),
                    ChimeraTK::logic_error);

  BOOST_CHECK_NO_THROW(
      ChimeraTK::BackendFactory::getInstance().loadPluginLibrary(
          "./libWrongVersionBackendCompat.so"));
  BOOST_CHECK_THROW(BackendFactory::getInstance().createBackend(
                        "sdm://./libWrongVersionBackendCompat=goodMapFile.map"),
                    ChimeraTK::logic_error);

  BOOST_CHECK_THROW(
      BackendFactory::getInstance().createBackend("(unregisteredBackend)"),
      ChimeraTK::logic_error);
  BOOST_CHECK_THROW(BackendFactory::getInstance().createBackend(
                        "sdm://./unregisteredBackend"),
                    ChimeraTK::logic_error);
}
...
BOOST_AUTO_TEST_CASE(testCreateFromUri) {
  ...
  boost::shared_ptr<DeviceBackend> testPtr;
  testPtr = BackendFactory::getInstance().createBackend(
      "sdm://./dummy=mtcadummy.map"); // get some dummy
