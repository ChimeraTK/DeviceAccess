...
BOOST_AUTO_TEST_CASE(test_toCooked_3_25) {
  ...
  BOOST_CHECK_EQUAL(converter.toCooked<std::string>(rawValue),
...
BOOST_AUTO_TEST_CASE(test_toCooked_60k) {
  ...
  BOOST_CHECK_THROW(converter.toCooked<int8_t>(rawValue),
                    ChimeraTK::logic_error);
  BOOST_CHECK_THROW(converter.toCooked<uint8_t>(rawValue),
                    ChimeraTK::logic_error);
...
BOOST_AUTO_TEST_CASE(test_toCooked_minus240) {
  ...
  BOOST_CHECK_THROW(converter.toCooked<uint16_t>(rawValue),
                    ChimeraTK::logic_error);
  BOOST_CHECK_EQUAL(converter.toCooked<int32_t>(rawValue), -241);
  BOOST_CHECK_THROW(converter.toCooked<uint32_t>(rawValue),
                    ChimeraTK::logic_error);
  BOOST_CHECK_EQUAL(converter.toCooked<int64_t>(rawValue), -241);
  BOOST_CHECK_THROW(converter.toCooked<uint64_t>(rawValue),
                    ChimeraTK::logic_error);
  BOOST_CHECK_EQUAL(converter.toCooked<std::string>(rawValue),
                    std::to_string(testValue));
}
...
BOOST_AUTO_TEST_CASE(test_from_3_25) {
  ...
  // corner cases
  BOOST_CHECK_THROW(converter.toRaw(std::string("notAFloat")),
                    ChimeraTK::logic_error);
