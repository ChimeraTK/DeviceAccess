...
public:
  static boost::chrono::steady_clock::time_point now() {
    std::cout << "TestIMPL:: returning now = " << (_epoch - _now).count()
              << std::endl;
    return _now;
  }
...
boost::chrono::steady_clock::time_point RebotTestableClock::_now =
...
boost::chrono::steady_clock::time_point
    RebotSleepSynchroniser::_nextRequestedWakeup = RebotTestableClock::_epoch;

namespace testable_rebot_sleep {
boost::chrono::steady_clock::time_point now() {
  std::cout << "function TestIMPL:: returning now = "
            << (RebotTestableClock::_now - RebotTestableClock::_epoch).count()
            << std::endl;
...
template <class Rep, class Period>
void advance_until(
    boost::chrono::duration<Rep, Period> targetTimeRelativeMyEpoch) {
  auto absoluteTargetTime =
      RebotTestableClock::_epoch + targetTimeRelativeMyEpoch;
  std::cout << "advanving to "
            << (absoluteTargetTime - RebotTestableClock::_epoch).count()
            << std::endl;
  std::cout << "next wakeup requested for "
            << (RebotSleepSynchroniser::_nextRequestedWakeup -
                RebotTestableClock::_epoch)
                   .count()
            << std::endl;
