...
RebotDummyServer::RebotDummyServer(unsigned int portNumber, std::string mapFile,
    ...
    : _state(ACCEPT_NEW_COMMAND), _heartbeatCount(0), _helloCount(0),
      _dont_answer(false), _registerSpace(mapFile), _serverPort(portNumber),
...
void RebotDummyServer::processReceivedPackage(std::vector<uint32_t> &buffer) {
  if (_state == INSIDE_MULTI_WORD_WRITE) {
    ...
    case PING:
      ...
      break;
    default:
      std::cout << "Instruction unknown in all protocol versions "
                << requestedAction << std::endl;
...
void RebotDummyServer::stop() {
  ...
  boost::asio::ip::tcp::endpoint endpoint(
      boost::asio::ip::address::from_string("127.0.0.1"), _serverPort);
