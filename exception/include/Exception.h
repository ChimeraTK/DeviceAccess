...
namespace ChimeraTK {
  ...
  /**
   *  Exception thrown when a runtime error has occured. Runtime errors are e.g. communication errors or hardware
   *  failures which can occur at any time. Those errors are therefore not detectable by other means. Typically it is
   *  possible to recover from a runtime_error (after the root cause of the error has been resolved) e.g. simply by
  ...
  /**
   *  Exception thrown when a logic error has occured. This usually means that the program logic is flawed, which points
   *  to a programming or configuration error. The exception is also thrown if a feature is used which is not
   *  implemented (e.g. an unsupported AccessModeFlag has been specified). After the exception, the system might be in
   *  an unspecified condition which might require shutting down the application.
   *
   *  Note, that it should be generally possible to avoid thrown logic_error exceptions in the first place by checking
   *  the system status and only perfoming only allowed operations. Therefore it is good practice to not catch this
