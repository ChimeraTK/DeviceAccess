...
* A <i>process variable</i> is the logical entity which is accessed through the TransferElement. Outside this document, it is sometimes also called register.
  ...
  * A process variable as a data type. The ChimeraTK framework supports the following types: [TODO]
...
* The <i>application buffer</i> (sometimes also called user buffer outside this document) is referring to the buffer containing the data and meta data, which is accessible to the application.
  ...
  * If not stated otherwise, the term refers to **all** components of the buffer.
...
* 1. Modes of transfers
  * 1.1 Read operations:
    * 1.1.1 The flag ChimeraTK::AccessMode::wait_for_new_data determines whether the transfer is initiated by the sender side (flag is set) or not.
    * 1.1.2 If ChimeraTK::AccessMode::wait_for_new_data is not set, read operations
      * obtain the <i>current</i> value of the process variable (if possible/applicable by communicating with the device),
      * have no information whether the value has changed, and
      * behave identical whether read(), readNonBlocking() or readLatest() is called, and readNonBlocking() always returns true.
    * 1.1.3 If ChimeraTK::AccessMode::wait_for_new_data is set,
      ...
      * readLatest() behaves identical as if ChimeraTK::AccessMode::wait_for_new_data had not been set.
      
  * 1.2 Write operations
    * 1.2.1 do not distingish on which end the transfer is initiated. The API allows for sender-initiated transfers and is compatible with receiver-initiated transfers as well.
    * 1.2.2 can optionally be "destructively", which allows the implementation to destroy content of the application buffer in the process.
...
* 2. Stages of an operation initiated by calling xxxYyy()
  * 2.1 preXxx(): calls doPreXxx() of the implementation to allow preparatory work before the actual transfer (see also 4). Might be empty, if nothing is to be done.
    * 2.1.1 In case of reads with AccessMode::wait_for_new_data (not readLatest()), preRead() and hece doPreRead() is not called before the actual transfer due to its asynchronous nature (initiated by the sender). Hence backend implementations usually will do nothing there, but decorator-like implementations still can use it to execute preparatory tasks.
  * 2.2 xxxTransferYyy(): This stage only exists for writes, for reads without AccessMode::wait_for_new_data, and for readLatest().
    * 2.2.1 Calls doWriteTransferYyy() resp. doReadTransferLatest(), which performs the actual transfer, but does not yet change the application buffer.
  * 2.3 postXxx(): calls doPostXxx() of the implementation to allow follow-up work after the actual transfer (see also 4). Might be empty, if nothing is to be done.
    * 2.3.1 doPostRead() is the only place where the application buffer maybe changed.

* 3. preXxx() and postXxx(), resp. doPreXxx() and doPostXxx(), are called always in pairs. (*)
  * 3.1 This holds even if exceptions (both ChimeraTK::logic_error and ChimeraTK::runtime_error, and also boost::thread_interrupted and boost::numeric::bad_numeric_cast) are thrown.
  * 3.2 The implementations of preXxx() and postXxx() ignores duplicate calls, such that a call to doPreXxx() is never followed by another call to doPreXxx() before doPostXxx() has been called, and vice versa.
  
* 4. Exceptions thrown in preXxx() or xxxTransferYyy() are caught and delayed until postXxx() by the framework. This ensures that preXxx() and postXxx() are always called in pairs.
  * 4.1 If in preXxx() an exception is thrown, the corresponding xxxTransferYyy() is not called, instead directly postXxx() is called.

* 5. Return values of xxxTransferYyy():
  * 5.1 readTransferNonBlocking() returns whether there has been new data. It returns true, if readTransfer() would not have blocked, and false otherwise.
  * 5.2 writeTransfer() and writeTransferDestructively return whether there has been data lost. If it returns true, some data was rejected in the process of the transfer. It can be either the data that should have been written in the current transfer, or some older data was overwritten.
  * 5.3 The return value is passed on to the postXxx() function call (via hasNewData resp. dataLost), to allow the doPostXxx() implementations to decide the right actions.
  * 5.4 In case of an exception in either preXxx() or xxxTransferYyy(), postXxx() receives instead the information that the transfer didn't sucessfully take place (hasNewData = false resp. dataLost = true).
  
* 6. read() and readNonBlocking() with AccessMode::wait_for_new_data:
  * 6.1 Since the transfer is initiated by the sender side in this case, the transfer is asynchronous to the read operation.
  * 6.2 The backend fills any received values into a queue, from which the read()/readNonBlocking() operations will obtain the value.
  * 6.3 Runtime errors like broken connections are reported by the backend by putting ChimeraTK::runtime_error exceptions in the queue. The exception will then be obtained by the read operation in place of a value.
  * 6.4 readLatest() uses a different mode of transfer which is initiated by the application (if possible/applicable).
    * 6.4.1 readLatest() does not remove entries from the queue, hence the same value will be read at some point by read()/readNonBlocking() as well.
    * 6.4.2 A certain inconsistency about the <i>current</i> value may appear, since there might be timing races between the modes. A value read by readLatest() might be arrive through a call to read()/readNonBlocking() only a certain time later.
    * 6.4.3 readLatest() never reads a value which is older then a value previously read by read()/readNonBlocking().
...
* 3. Reason: It might be that the user buffer has to be swapped out during the transfer (while taking away the ownership of the calling code), and this must be restored in the postXxx action.
...
* In doPreXxx, *only* ChimeraTK::logic_error must be thrown. In doPreWrite, boost::numeric::bad_numeric_cast can in addition be thrown.
...
## F. Notes about required changes

* There is no TransferFuture any more, use the TransferElement directly, instead!

### ControlSystemAdapter

* Implement a true readLatest() functionality: put value into atomic before pushing to queue. readLatest() reads from the atomic. How to do that with vectors? Need a second lockfree queue for that? Is it overengineering???


### ApplicationCore

* For the TestableModeAccessorDecorator: TransferElement::transferFutureWaitCallback() does no longer exist. Instead preRead() will now be called. In case of readAny, multiple calls to preRead() on different accessors will be made on the first call to readAny - the decorator has to deal with that properly.
