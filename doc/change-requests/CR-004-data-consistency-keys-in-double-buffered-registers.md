...
`ExceptionDummy` backend factory. No production code change.
...
- Add two scalar key registers. Keys must be scalar because the key accessor is
  ...
  - `TEST.KEYDB`, double-buffered with its own buffers but the same control
    ...
    Requirements).
- In `testDataConsistencyRealm.cpp`, add one test case that runs both key
  configurations from the same function body, printing which of the two
  scenarios is currently run. Each scenario opens the backend with a
  `DataConsistencyKeys` CDD mapping the respective key register to a realm,
  then drives the firmware side once: write key and data into buffer 1, set
  the inactive-buffer id to 0 so buffer 1 becomes the freshly finished buffer,
  and raise the interrupt.
- Each test writes a distinguishable value into the buffer to be read
  (buffer 1) only; the other buffer keeps whatever it held before (zeros or
  previous test data), so a wrong buffer fails the check. It asserts the
  delivered data equals the freshly written buffer and the `VersionNumber`
  equals `realm->getVersion(key)`; the correlated test additionally asserts the
  key value is the one of the same buffer generation as the data.
...
- One test case in the data-consistency test executable that runs both the
  plain-key and the correlated-key configuration in sequence, performing one
  buffer finish (delivered from buffer 1) as described in the Specifications;
  a `std::cout` line states which scenario is currently run. The test fails if
  either double buffering or the data consistency key handling is turned off.
