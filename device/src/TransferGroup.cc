...
void TransferGroup::addAccessor(TransferElementAbstractor &accessor) {
  ...
  if (accessor.getHighLevelImplElement()->isInTransferGroup) {
    throw ChimeraTK::logic_error(
        "The given accessor is already in a TransferGroup and cannot be added "
        "to another.");
  }
  ...
  for (auto &hlElem1 : highLevelElementsWithNewAccessor) {
    ...
    for (auto &replacement : list) {
      ...
      for (auto &hlElem : highLevelElementsWithNewAccessor) {
        hlElem->replaceTransferElement(
            replacement); // note: this does nothing, if the replacement cannot
  ...
  for (auto &hlElem : highLevelElements) {
    for (auto &hwElem : hlElem->getHardwareAccessingElements())
      lowLevelElements.insert(hwElem);
  }

  // update the list of CopyRegisterDecorators
  copyDecorators.clear();
  for (auto &hlElem : highLevelElements) {
    if (boost::dynamic_pointer_cast<ChimeraTK::CopyRegisterDecoratorTrait>(
            hlElem) != nullptr) {
...
struct TransferGroupTransferElementAbstractor : TransferElementAbstractor {
  TransferGroupTransferElementAbstractor(
      boost::shared_ptr<TransferElement> impl)
...
void TransferGroup::addAccessor(
