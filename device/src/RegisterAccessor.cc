...
RegisterAccessor::RegisterAccessor(
    ...
    : _registerPathName(registerPathName), _backend(deviceBackendPointer),
      _registerInfo(
          _backend->getRegisterCatalogue().getRegister(registerPathName)) {
  std::cerr << "***************************************************************"
               "**********************************"
            << std::endl; // LCOV_EXCL_LINE
  std::cerr << "** Class RegisterAccessor is deprecated and will be removed "
               "soon.                              **"
            << std::endl; // LCOV_EXCL_LINE
  std::cerr << "** Use OneDRegisterAccessor instead!                           "
               "                                **"
            << std::endl; // LCOV_EXCL_LINE
  std::cerr << "***************************************************************"
               "**********************************"
...
void RegisterAccessor::writeDMA(int32_t const *data, size_t dataSize,
                                uint32_t addRegOffset) { // LCOV_EXCL_LINE
