...
FixedPointConverter::FixedPointConverter(std::string variableName,
                                         unsigned int nBits, int fractionalBits,
                                         bool isSignedFlag)
    : _variableName(variableName), _nBits(nBits),
      _fractionalBits(fractionalBits), _isSigned(isSignedFlag),
...
void FixedPointConverter::reconfigure(unsigned int nBits, int fractionalBits,
                                      bool isSignedFlag) {
  _nBits = nBits;
  ...
  if ((fractionalBits >
       -std::numeric_limits<double>::min_exponent - static_cast<int>(nBits)) ||
      (fractionalBits <
       -std::numeric_limits<double>::max_exponent + static_cast<int>(nBits))) {
    std::stringstream errorMessage;
  ...
  if (nBits > 0) {
    _signBitMask = (_isSigned ? 1 << (nBits - 1)
                              : 0x0); // the highest valid bit is the sign
  } else {
    _signBitMask = 0;
  }
  ...
  _usedBitsMask =
      (1L << nBits) - 1L; // by using 1L (64 bit) this also works for 32 bits
  ...
  _maxRawValue =
      _usedBitsMask ^ _signBitMask; // bitwise xor: first bit is 0 if signed
...
template <>
std::string
FixedPointConverter::toCooked<std::string>(uint32_t rawValue) const {
...
template <>
uint32_t
FixedPointConverter::toRaw<std::string>(std::string cookedValue) const {
  if (_fractionalBits == 0) { // use integer conversion
    if (_isSigned) {
      return toRaw(std::stoi(cookedValue));
    } else {
      return toRaw(static_cast<uint32_t>(
          std::stoul(cookedValue))); // on some compilers, long might be a
