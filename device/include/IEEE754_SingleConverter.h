...
struct RoundingRangeCheckingDataConverter {
  ...
  typedef boost::numeric::converter<
      DestType, SourceType,
      boost::numeric::conversion_traits<DestType, SourceType>,
...
struct IEEE754_SingleConverter {
  template <typename CookedType> CookedType toCooked(uint32_t rawValue) const;

  template <typename CookedType> uint32_t toRaw(CookedType cookedValue) const;
...
CookedType IEEE754_SingleConverter::toCooked(uint32_t rawValue) const {
  ...
  try {
    cooked = RoundingRangeCheckingDataConverter<
        float, CookedType>::converter::convert(genericRepresentation);
...
uint32_t IEEE754_SingleConverter::toRaw(CookedType cookedValue) const {
  ...
  try {
    genericRepresentation = RoundingRangeCheckingDataConverter<
        CookedType, float>::converter::convert(cookedValue);
  } catch (boost::numeric::positive_overflow &) {
...
template <>
std::string
IEEE754_SingleConverter::toCooked<std::string>(uint32_t rawValue) const;
