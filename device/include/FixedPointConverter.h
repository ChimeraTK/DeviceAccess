  ...
  class FixedPointConverter {
   public:
    ...
    double toDouble(uint32_t rawValue) const {
      // std::cerr << " *** You are using the deprecated function ChimeraTK::FixedPointConverter::toDouble()." <<
      // std::endl; std::cerr << "    Please use toCooked() instead." << std::endl;
    ...
    uint32_t toFixedPoint(double cookedValue) const {
      // std::cerr << " *** You are using the deprecated function ChimeraTK::FixedPointConverter::toFixedPoint()." <<
      // std::endl; std::cerr << "    Please use toRaw() instead." << std::endl;
   ...
   private:
    ...
    uint32_t _bitShiftMaskSigned; ///< Mask with N most significant bits set, where N is the number of factional bits +
                                  ///< 1 if signed
    ...
    /** Internal exceptions to overload the what() function of the boost exceptions in order to fill in the variable
     * name. We derrive from the original exception so we do not break the signature in case existing code is catching
     * the exception. These exceptions are not part of the external interface and cannot be caught explicitly because
     * they are private. Catch boost::numeric::bad_numeric_cast and derrivatives if you want to do error handling.
     */
