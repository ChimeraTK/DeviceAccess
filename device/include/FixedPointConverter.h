...
public:
   ...
   */
  FixedPointConverter(std::string variableName, unsigned int nBits = 32,
                      int fractionalBits = 0, bool isSignedFlag = true);
   ...
   */
  template <typename UserType> UserType toCooked(uint32_t rawValue) const;
   ...
   * bits is represented.
   */
  template <typename UserType> uint32_t toRaw(UserType cookedValue) const;
  ...
  void reconfigure(unsigned int nBits = 32, int fractionalBits = 0,
  ...
  bool operator==(const FixedPointConverter &other) const {
    return _nBits == other._nBits && _fractionalBits == other._fractionalBits &&
...
private:
  ...
  uint32_t
      _bitShiftMaskSigned; ///< Mask with N most significant bits set, where N
  ...
  public:
    ...
    template <typename Pair> void operator()(Pair) const {
      ...
      try {
        ...
        boost::fusion::at_key<UserType>(_fpc->_minCookedValues) =
            std::numeric_limits<UserType>::min();
      }
      try {
        ...
        boost::fusion::at_key<UserType>(_fpc->_maxCookedValues) =
            std::numeric_limits<UserType>::max();
      }
    }
  ...
  template <typename UserType,
            typename std::enable_if<std::is_signed<UserType>{}, int>::type = 0>
  ...
  struct FPC_OverflowException : public BOOST_EXCEPTION_T {
    FPC_OverflowException(std::string variableName)
        : errorMessage("Exception during fixed point conversion in " +
...
UserType FixedPointConverter::toCooked(uint32_t rawValue) const {
  ...
  if (isNegative)
    rawValue = ~rawValue; // bit inversion flips the sign, but will be off by 1!
  ...
  if (std::numeric_limits<UserType>::is_integer && _fractionalBits == 0) {
    ...
    try {
      ...
      if (!isNegative) {
        throw FPC_OverflowException<boost::numeric::positive_overflow>(
            _variableName);
      } else {
        throw FPC_OverflowException<boost::numeric::negative_overflow>(
            _variableName);
      }
    }
    ...
    typedef boost::numeric::converter<
        UserType, double, boost::numeric::conversion_traits<UserType, double>,
...
uint32_t FixedPointConverter::toRaw(UserType cookedValue) const {
  ...
  if (std::numeric_limits<UserType>::is_integer && _fractionalBits == 0) {
    ...
    double d_cooked =
        _inverseFractionalBitsCoefficient * static_cast<double>(cookedValue);
    ...
    try {
      if (_isSigned) {
        typedef boost::numeric::converter<
            int32_t, double, boost::numeric::conversion_traits<int32_t, double>,
            boost::numeric::def_overflow_handler, Round<double>>
            converter_signed;
        ...
        typedef boost::numeric::converter<
            uint32_t, double,
            boost::numeric::conversion_traits<uint32_t, double>,
            boost::numeric::def_overflow_handler, Round<double>>
            converter_unsigned;
...
template <typename UserType,
          typename std::enable_if<std::is_signed<UserType>{}, int>::type>
