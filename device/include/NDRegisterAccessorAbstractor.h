...
public:
  ...
  const NDRegisterAccessorAbstractor &
  operator=(const NDRegisterAccessorAbstractor &rightHandSide) const = delete;
...
protected:
  ...
  NDRegisterAccessor<UserType> *get() {
    return static_cast<NDRegisterAccessor<UserType> *>(
        TransferElementAbstractor::_impl.get());
  }
