...
namespace ChimeraTK {
  ...
  /**
   * Class for generating and holding version numbers without exposing a numeric
   * representation
   *
   * Version numbers are used to resolve competing updates that are applied to the
   * same process variable. For example, it they can help in breaking an infinite
   * update loop that might occur when two process variables are related and
   * update each other.
   *
   * They are also used to determine the order of updates made to different
   * process variables.
   */
  class VersionNumber {
   public:
    /** Default constructor: Generate new unique version number with current time
     * as time stamp */
     ...
     *
     *  Compare version number only, since they are ordered in time and atomically
     * generated, so the result is logically as expected. The time stamp is not
     * precise and not atomically generated, so comparing it would not be precise.
     */
    ...
    /** Conversion into a human readable std::string to allow e.g. printing the
     * version number on screen. Do not try to parse the string in any way, the
   ...
   private:
     ...
     * an atomic way, so that it is guaranteed that this method never returns the
     * same version number twice (unless the counter overflows, which is very
     * unlikely). The first version number returned by this method is one. The
     * version number that is returned is guaranteed to be greater than the
     * version numbers returned for earlier calls to this method. This method may
