...
public:
  ...
  void read(ConvertedDataType *convertedData, size_t nWords = 1,
    ...
    auto &accessorHandler = boost::fusion::at_key<ConvertedDataType>(
        _convertingAccessorHandlers.table);
    accessorHandler.checkAndResize(nWords, wordOffsetInRegister,
                                   false /*not raw*/, _backend,
                                   _registerPathName);
    // now we can be sure that the accessor is valid and has enough memory to
    // perform the memcpy
    accessorHandler.accessor->read();
    // copy data to target buffer. The accessor might not start at index 0, so
    // we have to calculate the correct offset inside the accessor
    memcpy(convertedData,
           accessorHandler.accessor->accessChannel(0).data() +
               wordOffsetInRegister - accessorHandler.beginIndex,
  ...
  void write(ConvertedDataType const *convertedData, size_t nWords,
    ...
    accessorHandler.checkAndResize(nWords, wordOffsetInRegister,
                                   ...
                                   _registerPathName);
    // now we can be sure that the accessor is valid and has enough memory to
    // perform the memcpy copy data from source buffer to the correct place in
    // the accessor. The accessor does not necessarily start at the beginnig of
    // the register, so we have to correct the offset for this.
    memcpy(accessorHandler.accessor->accessChannel(0).data() +
               wordOffsetInRegister - accessorHandler.beginIndex,
  ...
  RegisterInfoMap::RegisterInfo getRegisterInfo() const {
    ...
    auto castedBackend =
        boost::dynamic_pointer_cast<NumericAddressedBackend>(_backend);
    if (!castedBackend) {
      throw ChimeraTK::logic_error("RegisterAccessor::getRegisterInfo() called "
                                   "for a non-NumericAddressedBackend.");
    }
  ...
  void readRaw(int32_t *data, size_t dataSize = 0,
    ...
    if (dataSize == 0)
      dataSize = getNumberOfElements() * sizeof(int32_t);
    // check word alignment
    if (dataSize % 4 != 0 || addRegOffset % 4 != 0) {
      throw ChimeraTK::logic_error("RegisterAccessor::writeRaw with incorrect "
                                   "word alignment (size and offset must be "
                                   "dividable by 4)");
    }
    size_t nWords = dataSize / sizeof(int32_t);
    size_t wordOffsetInRegister = addRegOffset / sizeof(int32_t);

    _rawAccessorHandler.checkAndResize(nWords, wordOffsetInRegister,
                                       true /*raw*/, _backend,
                                       _registerPathName);
    // perform read
    ...
    memcpy(data,
           _rawAccessorHandler.accessor->accessChannel(0).data() +
               wordOffsetInRegister - _rawAccessorHandler.beginIndex,
           dataSize);
  }
  ...
  void writeRaw(int32_t const *data, size_t dataSize = 0,
    ...
    memcpy(_rawAccessorHandler.accessor->accessChannel(0).data() +
               wordOffsetInRegister - _rawAccessorHandler.beginIndex,
           data, dataSize);
    // perform write
...
protected:
  ...
  template <class UserType> class AccessorHandler {
  public:
    ...
    size_t endIndex; // the end index (first index after the valid range, liken
                     // end iterator)
    void checkAndResize(size_t nWords, size_t wordOffsetInRegister, bool isRaw,
                        boost::shared_ptr<DeviceBackend> const &backend,
                        RegisterPath const &registerPathName) {
      // no need for out of register size checking. getRegisterAccessor does the
      // job for us and throws in case of error.
      if ((wordOffsetInRegister < beginIndex) ||
          (wordOffsetInRegister + nWords > (endIndex) || !accessor)) {
        ...
        accessor = backend->getRegisterAccessor<UserType>(
            registerPathName, newEndIndex - newBeginIndex, newBeginIndex,
            isRaw); // 0 offset, raw
        // we have to update the accessor with the current hardware information.
        // It might be that a partial write is going to happen.
        accessor->read();
        // only if creating the new accessor succeeds we change the index
...
template <>
inline void RegisterAccessor::read<std::string>(std::string *, size_t,
                                                uint32_t) const {
  std::cout
      << "Do not use the deprecated mtca4u::RegisterAccessor with strings!"
      << std::endl;
  std::terminate();
}

template <>
inline void RegisterAccessor::write<std::string>(std::string const *, size_t,
                                                 uint32_t) {
  std::cout
      << "Do not use the deprecated mtca4u::RegisterAccessor with strings!"
      << std::endl;
