  ...
  class RegisterAccessor {
   public:
    ...
    void read(ConvertedDataType* convertedData, size_t nWords = 1, uint32_t wordOffsetInRegister = 0) const {
      ...
      accessorHandler.checkAndResize(nWords, wordOffsetInRegister, false /*not raw*/, _backend, _registerPathName);
      ...
      memcpy(convertedData,
          accessorHandler.accessor->accessChannel(0).data() + wordOffsetInRegister - accessorHandler.beginIndex,
    ...
    void readRaw(int32_t* data, size_t dataSize = 0, uint32_t addRegOffset = 0) const {
      ...
      _rawAccessorHandler.checkAndResize(nWords, wordOffsetInRegister, true /*raw*/, _backend, _registerPathName);
      // perform read
      _rawAccessorHandler.accessor->read();
      // copy to target buffer
      memcpy(data,
          _rawAccessorHandler.accessor->accessChannel(0).data() + wordOffsetInRegister - _rawAccessorHandler.beginIndex,
          dataSize);
    }
    ...
    class AccessorHandler {
     public:
      ...
      void checkAndResize(size_t nWords, size_t wordOffsetInRegister, bool isRaw,
          boost::shared_ptr<DeviceBackend> const& backend, RegisterPath const& registerPathName) {
