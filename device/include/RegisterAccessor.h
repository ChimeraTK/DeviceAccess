    ...
    void readRaw(int32_t* data, size_t dataSize = 0, uint32_t addRegOffset = 0) const {
      ...
      if(dataSize % 4 != 0 || addRegOffset % 4 != 0) {
        throw ChimeraTK::logic_error(
            "RegisterAccessor::writeRaw with incorrect word alignment (size and offset must be "
            "dividable by 4)");
      }
    ...
    class AccessorHandler {
     public:
      ...
      void checkAndResize(size_t nWords, size_t wordOffsetInRegister, bool isRaw,
        ...
        // no need for out of register size checking. getRegisterAccessor does the job for us and throws in case of
        // error.
        if((wordOffsetInRegister < beginIndex) || (wordOffsetInRegister + nWords > (endIndex) || !accessor)) {
          ...
          accessor = backend->getRegisterAccessor<UserType>(
              registerPathName, newEndIndex - newBeginIndex, newBeginIndex, isRaw); // 0 offset, raw
          // we have to update the accessor with the current hardware information. It might be that a partial write is
          // going to happen.
          accessor->read();
