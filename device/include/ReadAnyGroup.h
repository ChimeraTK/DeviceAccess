...
public:
  ...
  private:
    ...
    Notification(TransferElementAbstractor const &transferElement,
                 std::size_t index);
  ...
  template <typename ITERATOR> ReadAnyGroup(ITERATOR first, ITERATOR last);
...
inline ReadAnyGroup::Notification::Notification(Notification &&other)
    : accepted(other.accepted), index(other.index),
...
inline ReadAnyGroup::Notification &ReadAnyGroup::Notification::
operator=(Notification &&other) {
  // It is important that each received notification is consumed. This means
...
inline TransferElementAbstractor
ReadAnyGroup::Notification::getTransferElement() {
  if (!this->valid) {
    throw std::logic_error("This notification object is invalid.");
  }
...
inline ReadAnyGroup::Notification::Notification(
    TransferElementAbstractor const &transferElement_, std::size_t index_)
...
inline ReadAnyGroup::ReadAnyGroup(
    std::initializer_list<TransferElementAbstractor> list) {
...
inline ReadAnyGroup::ReadAnyGroup(
    std::initializer_list<boost::shared_ptr<TransferElement>> list) {
  for (auto &element : list)
    add(element);
  finalise();
}
...
inline void ReadAnyGroup::add(TransferElementAbstractor element) {
  ...
  if (!element.isReadable()) {
    throw std::logic_error("Cannot add non-readable accessor for register " +
                           element.getName() + " to ReadAnyGroup.");
  }
...
inline void
ReadAnyGroup::readUntilAll(const std::vector<TransferElementID> &ids) {
  ...
  while (true) {
    auto read = readAny();
    if (found.count(read) == 0)
      continue; // variable is not in the vector ids
    if (found[read] == true)
      continue; // variable has been read already
    found[read] = true;
    --leftToFind;
    if (leftToFind == 0)
      return;
...
inline void ReadAnyGroup::readUntilAll(
  ...
  while (true) {
    auto read = readAny();
    if (found.count(read) == 0)
      continue; // variable is not in the vector elements
    if (found[read] == true)
      continue; // variable has been read already
    found[read] = true;
    --leftToFind;
