...
namespace mtca4u {
   ...
   */
  template<typename T>
  class BufferingRegisterAccessor : public  NDRegisterAccessorBridge<T> {
    public:
      ...
      : NDRegisterAccessorBridge<T>(impl)
      {
        if(NDRegisterAccessorBridge<T>::_impl->getNumberOfChannels() != 1) {
      ...
      void read() {
        NDRegisterAccessorBridge<T>::_impl->read();
      }

      /** Convert data from the buffer and write to device. */
      void write() {
        NDRegisterAccessorBridge<T>::_impl->write();
      }
      ...
      T& operator[](unsigned int index) {
        return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0][index];
      }

      /** Return number of elements
       */
      unsigned int getNumberOfElements() {
        return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].size();
      }
      ...
      operator T&() {
        return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0][0];
      }

      /** Assignment operator, assigns the first element. */
      BufferingRegisterAccessor<T>& operator=(T rightHandSide)
      {
        NDRegisterAccessorBridge<T>::_impl->buffer_2D[0][0] = rightHandSide;
        return *this;
      }

      /** Pre-increment operator for the first element. */
      BufferingRegisterAccessor<T>& operator++() {
        return operator=( NDRegisterAccessorBridge<T>::_impl->buffer_2D[0][0] + 1 );
      }
      ...
      T operator++(int) {
        T v = NDRegisterAccessorBridge<T>::_impl->buffer_2D[0][0];
        operator=( v + 1 );
        return v;
      }
      ...
      iterator begin() { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].begin(); }
      const_iterator begin() const { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].cbegin(); }
      const_iterator cbegin() const { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].cbegin(); }
      iterator end() { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].end(); }
      const_iterator end() const { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].cend(); }
      const_iterator cend() const { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].cend(); }
      reverse_iterator rbegin() { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].rbegin(); }
      const_reverse_iterator rbegin() const { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].crbegin(); }
      const_reverse_iterator crbegin() const { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].crbegin(); }
      reverse_iterator rend() { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].rend(); }
      const_reverse_iterator rend() const { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].crend(); }
      const_reverse_iterator crend() const { return NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].crend(); }

      /* Swap content of (cooked) buffer with std::vector */
      void swap(std::vector<T> &x) {
        NDRegisterAccessorBridge<T>::_impl->buffer_2D[0].swap(x);
      }

      /** Return if the register accessor allows only reading */
      bool isReadOnly() const {
        return NDRegisterAccessorBridge<T>::_impl->isReadOnly();
      }
      ...
      bool isInitialised() const {
        return NDRegisterAccessorBridge<T>::_impl != NULL;
      }
