...
/*********************************************************************************************************************/
/** Accessor class to read and write registers transparently by using the
 * accessor object like a vector of the type UserType. Conversion to and from
 * the UserType will be handled by a data converter matching the register
 * description in the map (if applicable). Obtain the accessor using the
...
public:
   ...
   *  Users should call Device::getOneDRegisterAccessor() to obtain an instance
   * instead. */
   ...
   *  Note: Using the iterators is slightly more efficient than using this
   * operator! */
  ...
  typedef typename std::vector<UserType>::const_reverse_iterator
  ...
  const_reverse_iterator rbegin() const {
    return get()->accessChannel(0).crbegin();
  }
  ...
  const_reverse_iterator rend() const {
    return get()->accessChannel(0).crend();
  }
  ...
  void swap(std::vector<UserType> &x) {
    if (x.size() != get()->accessChannel(0).size()) {
      throw ChimeraTK::logic_error(
          "Swapping with a buffer of a different size is not allowed.");
    }
  ...
  OneDRegisterAccessor<UserType> &operator=(const std::vector<UserType> &x) {
    if (x.size() != get()->accessChannel(0).size()) {
      throw ChimeraTK::logic_error(
          "Copying in a buffer of a different size is not allowed.");
    }
   ...
   * write() and swap(). If this accessor is part of a TransferGroup, any call
   * to one of these functions on any element of the TransferGroup or the
   ...
   */
  template <typename COOKED_TYPE> COOKED_TYPE getAsCooked(unsigned int sample) {
