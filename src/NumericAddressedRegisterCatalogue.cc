...
namespace ChimeraTK {
  ...
  : pathName(pathName_), nElements(nElements_), elementPitchBits(nElements_ > 0 ? nBytes_ / nElements_ * 8 : 0),
    ...
    doubleBuffers(std::move(doubleBufferInfo_)), channels({{0, dataType_, width_, nFractionalBits_, signedFlag_}}) {
  ...
  NumericAddressedRegisterInfo::NumericAddressedRegisterInfo(RegisterPath const& pathName_, uint64_t bar_,
      ...
      Access dataAccess_, std::vector<size_t> interruptId_, std::vector<DoubleBufferInfo> doubleBufferInfo_)
  : pathName(pathName_), nElements(nElements_), elementPitchBits(elementPitchBits_), bar(bar_), address(address_),
    registerAccess(dataAccess_), interruptId(std::move(interruptId_)), doubleBuffers(std::move(doubleBufferInfo_)),
