...
#pragma GCC diagnostic pop
  ...
  boost::shared_ptr<NDRegisterAccessor<UserType>> NumericAddressedBackend::getSyncRegisterAccessor(
    ...
    if(registerInfo.doubleBuffer == std::nullopt) {
      ...
      accessor = boost::make_shared<DoubleBufferAccessorDecorator<UserType>>(*registerInfo.doubleBuffer,
          shared_from_this(), controlState, registerPathName, numberOfWords, wordOffsetInRegister, flags);
