...
namespace ChimeraTK {
  ...
  void DataConsistencyGroup::initMatcher() {
    if(_mode == MatchingMode::historized) {
      _impl = std::make_unique<HistorizedMatcher>();
    }
    else {
      _impl = std::make_unique<SimpleMatcher>();
    }
  };
...
#pragma GCC diagnostic pop
  ...
  void DataConsistencyGroup::add(TransferElementAbstractor& element, unsigned histLen) {
    checkAccess(element);
    if(_mode == MatchingMode::historized) {
      dynamic_cast<HistorizedMatcher*>(_impl.get())->add(element, histLen);
    }
    else {
      _impl->getElements()[element.getId()] = element;
    }
  ...
  bool DataConsistencyGroup::update(const TransferElementID& transferElementID) {
    ...
    switch(_mode) {
      ...
      case MatchingMode::exact:
        return dynamic_cast<SimpleMatcher*>(_impl.get())->update(transferElementID);
      case MatchingMode::historized:
        ...
        dynamic_cast<HistorizedMatcher*>(_impl.get())->updateCalled(transferElementID);
        return true;
    }
    assert(false);
    return false;
  ...
  const BaseMatcher& DataConsistencyGroup::getMatcher() {
    return *_impl;
  }
  ...
  bool DataConsistencyGroup::isConsistent() const {
    switch(_mode) {
      case MatchingMode::none:
        ...
        return true;
      case MatchingMode::exact:
        return dynamic_cast<SimpleMatcher*>(_impl.get())->isConsistent();
      case MatchingMode::historized:
