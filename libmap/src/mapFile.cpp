...
bool mapFile::check(errorList &err, mapFile::errorList::errorElem::TYPE level) {
    ...
    iter_n = iter_p + 1;
    while (1) {
        if ((*iter_p).reg_name == (*iter_n).reg_name && ((*iter_p).reg_address != (*iter_n).reg_address ||
                (*iter_p).reg_bar != (*iter_n).reg_bar || (*iter_p).reg_elem_nr != (*iter_n).reg_elem_nr || (*iter_p).reg_size != (*iter_n).reg_size)) {
            err.insert(errorList::errorElem(errorList::errorElem::ERROR, errorList::errorElem::NONUNIQUE_REGISTER_NAME, (*iter_p), (*iter_n), map_file_name));
            ret = false;
        }
        iter_p = iter_n;
        iter_n = ++iter_n;
    ...
    for (iter_p = map_file.begin(); iter_p != map_file.end(); iter_p++) {
        ...
        for (v_iter = v_addresses.begin(); v_iter != v_addresses.end(); v_iter++) {
            if ((*iter_p).reg_bar == (*((*v_iter).iter)).reg_bar) {
                if ((*v_iter).start >= (*iter_p).reg_address && (*v_iter).start < (*iter_p).reg_address + (*iter_p).reg_size) {
                    if (level >= errorList::errorElem::WARNING){
                        err.insert(errorList::errorElem(errorList::errorElem::WARNING, errorList::errorElem::WRONG_REGISTER_ADDRESSES, (*iter_p), (*((*v_iter).iter)), map_file_name));
                        ret = false;
                    }
...
std::ostream& operator<<(std::ostream &os, const mapFile::mapElem& me) {
     os << me.reg_name << " 0x" << std::hex << me.reg_elem_nr << " 0x" << me.reg_address << " 0x" << me.reg_size << " 0x" << me.reg_bar << std::dec;
    return os;
}
